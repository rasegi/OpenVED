...
public:
  ...
  void DrawLine(const TDMatLine *, bool) override { ++lineDraws; }
...
public:
  explicit TestViewInterface(TDGraphicEngine *graphicEngine) { SetGraphicEngine(graphicEngine); }
...
int main() {
  {
    ...
    if (int result =
            expect(p2 && nearlyEqual(p2->x, 100.0) && nearlyEqual(p2->y, 0.0), "polygon second point mismatch")) {
      return result;
    }
  }

  {
    ...
    if (int result = expect(graphicEngine.constructPolygonDraws == 1,
                            "bspline first-click preview did not keep a temporary control polygon alive")) {
      return result;
    }
  }

  {
    ...
    const auto previewIt =
        std::find_if(graphicEngine.constructPolygons.begin(), graphicEngine.constructPolygons.end(),
                     [](const std::vector<TDMatPoint> &points) {
                       return points.size() == 4 && nearlyEqual(points[1].x, 60.0) && nearlyEqual(points[1].y, 30.0);
                     });
    if (int result = expect(previewIt != graphicEngine.constructPolygons.end(),
                            "bspline insert preview moved the wrong node: " +
                                constructSummary(graphicEngine.constructPolygons))) {
      return result;
    }
    ...
    if (int result = expect(finalPoint && nearlyEqual(finalPoint->x, previewPoint.x) &&
                                nearlyEqual(finalPoint->y, previewPoint.y),
...

  {
    ...
    if (int result =
            expect(graphicEngine.lastPolygonPointCount == 3, "polygon preview does not include the moving endpoint")) {
      return result;
    }
  }

  {
    ...
    editor.PaintContentOnView(&view);
    if (int result = expect(graphicEngine.polygonDraws == 1,
                            "bezier first-click preview did not keep a temporary polygon alive")) {
      return result;
    }
    if (int result =
            expect(graphicEngine.lastPolygonPointCount == 1, "bezier first-click preview has wrong point count")) {
      return result;
    }
  }

  {
    ...
    editor.PaintContentOnView(&view);
    if (int result =
            expect(graphicEngine.polygonDraws == 1, "bezier moving preview did not draw the control polygon")) {
      return result;
    }
    if (int result = expect(graphicEngine.lineDraws == 0, "bezier moving preview still draws only a temporary line")) {
      return result;
    }
    if (int result = expect(graphicEngine.lastPolygonPointCount == 3,
                            "bezier moving preview does not include fixed points and moving endpoint")) {
      return result;
    }
  }
...

  {
    ...
    if (int result =
            expect(polyCurve->GetResolution() == 9 && !polyCurve->GetShowControls() && !polyCurve->GetShowPolygon(),
...

  {
    ...
    if (int result =
            expect(control && control->eType == CPT_PRIME_QSPLINE, "polycurve second point is not QSPLINE control")) {
      return result;
    }
  }
