...
int main() {
  ...
  if (int result = expect(operationManager.GetAktiveOperationType() == VOM_ROTATE_OBJECT_ACTIVEPOINT,
                          "rotate operation was not activated")) {
    return result;
  }
  ...
  if (int result = expect(operationManager.GetAktiveOperationType() == VOM_ROTATE_OBJECT_ACTIVEPOINT,
                          "rotate operation changed after first copy rotate")) {
    return result;
  }
  if (int result = expect(model.CountObjects() == 2, "first copy rotate did not append one clone")) {
    return result;
  }
  if (int result = expect(model.CountSelectedObjects() == 1,
                          "select copy did not leave exactly one selected object after first rotate")) {
    return result;
  }
