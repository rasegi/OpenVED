...
int main() {
  ...
  if (int result = expect(std::abs(rightEdgeState.RealToXPos(rightAnchorX) - rightMouseX) <= 1,
                          "right edge zoom in lost anchored x")) {
    return result;
  }
  if (int result = expect(std::abs(rightEdgeState.RealToYPos(rightAnchorY) - rightMouseY) <= 1,
                          "right edge zoom in lost anchored y")) {
    return result;
  }
  ...
  if (int result = expect(std::fabs(panState.GetViewRange().Left - desiredLeft) <= std::fabs(panPerPixelX),
                          "free pan left clamped at world edge")) {
    return result;
  }
  if (int result = expect(std::fabs(panState.GetViewRange().Top - desiredTop) <= std::fabs(panPerPixelY),
                          "free pan top clamped at world edge")) {
    return result;
  }
