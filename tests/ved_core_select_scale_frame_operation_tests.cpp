...
int expectLine(const TDVecLine &line, double x1, double y1, double x2, double y2, const std::string &label) {
  ...
  if (nearlyEqual(actual.x1, x1) && nearlyEqual(actual.y1, y1) && nearlyEqual(actual.x2, x2) &&
      nearlyEqual(actual.y2, y2)) {
    return 0;
  }

  std::cerr << label << ": got (" << actual.x1 << ", " << actual.y1 << ") -> (" << actual.x2 << ", " << actual.y2
...
int expectEllipse(const TDVecEllipse &ellipse, double xCenter, double yCenter, double xRadius, double yRadius,
  ...
  if (nearlyEqual(actual.xCenter, xCenter) && nearlyEqual(actual.yCenter, yCenter) &&
      nearlyEqual(actual.xRadius, xRadius) && nearlyEqual(actual.yRadius, yRadius) &&
      nearlyEqual(actual.nAngle, angle)) {
    return 0;
  }

  std::cerr << label << ": got center (" << actual.xCenter << ", " << actual.yCenter << "), radii (" << actual.xRadius
            << ", " << actual.yRadius << "), angle " << actual.nAngle << '\n';
  return 1;
}
...
int main() {
  {
    ...
    if (int result = expect(operationManager.GetAktiveOperationType() == VOM_SELECTMOVE_OBJ_SCALE_FRAME,
                            "select-scale operation was not activated")) {
      return result;
    }
    if (int result = expect(!editor.GetVisualNodes() && editor.GetVisualFrame(),
                            "select-scale did not switch to frame visualization")) {
      return result;
    }
...

  {
    ...
    if (int result =
            expect(!nearlyEqual(objectFrame.P1.y, objectFrame.P2.y), "rotated ellipse object frame was not oriented")) {
      return result;
    }
    if (int result = expect(nearlyEqual(scaleFrame.P1.x, -extent) && nearlyEqual(scaleFrame.P1.y, -extent) &&
                                nearlyEqual(scaleFrame.P3.x, extent) && nearlyEqual(scaleFrame.P3.y, extent),
    ...
    if (int result =
            expect(ellipse.GetXRadius() > 0.0 && ellipse.GetYRadius() > 0.0, "ellipse scale produced invalid radii")) {
      return result;
    }
...

  {
    ...
    if (int result = expectLine(*line, 10.0, 10.0, 110.0, 10.0,
                                "first selected line was not moved by select-scale group move")) {
      return result;
    }
    if (int result = expectLine(*secondLine, 10.0, 30.0, 110.0, 30.0,
                                "second selected line was not moved by select-scale group move")) {
      return result;
    }
  }
...

  {
    ...
    if (int result =
            expectEllipse(circle, 5.0, 0.0, 15.0, 15.0, 0.0, "circle did not stay circle after select-frame move")) {
      return result;
    }
  }
