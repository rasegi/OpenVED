...
public:
  explicit TestFontProvider(std::vector<std::string> fontIds) : fontIds_(std::move(fontIds)) {}
...
bool nearlyEqual(double left, double right) { return std::fabs(left - right) < kEpsilon; }
...
int main() {
  ...
  // DocumentSettings roundtrip: Custom format with arbitrary size
  {
    TDVecModel model;
    TDVecDocumentSettings settings;
    settings.pageSettings = TDVecPageFormats::Custom("Banner", 2000000.0, 500000.0, TDVecPageOrientation::Landscape);
    model.SetDocumentSettings(settings);
