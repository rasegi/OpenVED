...
int main() {
  ...
  assert(file);
  const std::vector<char> original((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
  assert(original.size() > 1028);

  std::unique_ptr<TDVecFont> font = LoadVecFontFromMemory(original.data(), static_cast<long>(original.size()));
  assert(font);
  ...
  std::unique_ptr<TDVecFont> reloaded = LoadVecFontFromMemory(saved.data(), static_cast<long>(saved.size()));
  assert(reloaded);
