...
int main() {
  {
    ...
    if (int result =
            expect(first && nearlyEqual(first->x, 10.0) && nearlyEqual(first->y, -5.0), "bezier move mismatch")) {
      return result;
    }
  }

  {
    ...
    if (int result = expect(bspline.GetDegree() == 2 && bspline.GetResolution() == 50, "bspline defaults mismatch")) {
      return result;
    }
