...
int expectLine(const TDVecLine &line, double x1, double y1, double x2, double y2, const std::string &label) {
  ...
  return expect(nearlyEqual(actual.x1, x1) && nearlyEqual(actual.y1, y1) && nearlyEqual(actual.x2, x2) &&
...
int main() {
  ...
  if (int result = expect(operationManager.GetAktiveOperationType() == VOM_SELECT_MOVE_NODE_OBJECT,
                          "select-move operation was not activated")) {
    return result;
  }
  ...
  if (int result =
          expect(!line->GetSelect() && secondLine->GetSelect(), "area select did not select only second line")) {
    return result;
  }
  ...
  if (int result =
          expectLine(*movedFirst, 10.0, 10.0, 110.0, 10.0, "group first line was not moved through select operation")) {
    return result;
  }
  if (int result = expectLine(*movedSecond, 10.0, 30.0, 110.0, 30.0,
                              "group second line was not moved through select operation")) {
    return result;
  }
  ...
  auto *groupNodeSecondLine =
      dynamic_cast<TDVecLine *>(groupNodeModel.AppendObject(new TDVecLine(0.0, 20.0, 100.0, 20.0)));
  ...
  if (int result =
          expectLine(*scaledFirst, 0.0, 0.0, 200.0, 0.0, "group first line was not scaled through node move")) {
    return result;
  }
  if (int result =
          expectLine(*scaledSecond, 0.0, 40.0, 200.0, 40.0, "group second line was not scaled through node move")) {
    return result;
  }
