...
int main() {
  ...
  std::unique_ptr<TDVecFont> font = ved::fontconvert::ConvertTrueTypeFileToVecFont(ttf, 0, name);
  assert(font);
  ...
  std::unique_ptr<TDVecFont> reloaded = LoadVecFontFromMemory(bytes.data(), static_cast<long>(bytes.size()));
  assert(reloaded);
  ...
  std::unique_ptr<TDVecFont> arabic = ved::fontconvert::ConvertTrueTypeFileToVecFont(
      "third_party/fonts/amiri/Amiri-Regular.ttf", 0, "VC:Amiri", ved::fontconvert::CharacterCoverage::FullCmap);
  assert(arabic);
