...
bool approxEqual(double a, double b) { return std::fabs(a - b) < kEpsilon; }
...
void testCalculateForViewHighZoom() {
  ...
  const TDVecMeasureScale scale = calc.CalculateForView(1.0, 1, 10000.0, 10);
...
void testCalculateForViewLowZoom() {
  TDVecMeasureScaleCalculator calc(TDVecUnitSettings{});
  const TDVecMeasureScale scale = calc.CalculateForView(50.0, 1, 10000.0, 10);
...
void testCalculateForViewVeryLowZoom() {
  TDVecMeasureScaleCalculator calc(TDVecUnitSettings{});
  const TDVecMeasureScale scale = calc.CalculateForView(500.0, 1, 10000.0, 10);
...
void testCalculateForViewMinorNeverExceedsMajor() {
  ...
  for (double rpp : testRealPerPixel) {
    const TDVecMeasureScale scale = calc.CalculateForView(rpp, 1, 10000.0, 10);
