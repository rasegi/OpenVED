...
public:
  ...
  void DrawNode(double, double, TDNodeType, bool) override { ++nodeDraws; }
...
public:
  ...
  void Send_ViewRefresh_ToView() override { ++refreshCount; }
...
int main() {
  ...
  if (int result = expect(graphicEngine.nodeDraws == 1, "scale 3 point did not keep first interactive node: " +
                                                            std::to_string(graphicEngine.nodeDraws))) {
    return result;
  }
  ...
  editor.PaintContentOnView(&view);
  if (int result =
          expect(graphicEngine.nodeDraws == 2, "scale 3 point did not keep first and second interactive nodes: " +
                                                   std::to_string(graphicEngine.nodeDraws))) {
    return result;
  }
  ...
  graphicEngine.nodeDraws = 0;
  editor.PaintContentOnView(&view);
  if (int result = expect(graphicEngine.nodeDraws == 0, "scale 3 point did not clear temporary nodes after finish: " +
                                                            std::to_string(graphicEngine.nodeDraws))) {
    return result;
  }
