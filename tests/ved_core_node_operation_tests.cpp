...
TDMatConturPoint conturPoint(double x, double y, TDConturPointType type = CPT_PRIME_LINE) { return {x, y, type, true}; }
...
int main() {
...

  {
    ...
    operationManager.SetOperation(VOM_MOVE_NODE);
    if (int result =
            expect(editor.GetUsedCursor() == VECVIEW_SIMPLE_CROSS, "move node did not use the simple cross cursor")) {
      return result;
    }
...

  {
    ...
    operationManager.SetOperation(VOM_INSERT_NODE);
    if (int result =
            expect(editor.GetUsedCursor() == VECVIEW_CROSS_PLUS, "insert node did not use the cross plus cursor")) {
      return result;
    }
...

  {
    ...
    operationManager.SetOperation(VOM_DELETE_NODE);
    if (int result =
            expect(editor.GetUsedCursor() == VECVIEW_CROSS_MINUS, "delete node did not use the cross minus cursor")) {
      return result;
    }
    operationManager.MouseDown(VIRTMOUSEBTM_LEFT, VKState_KEY_UNKNOWN, 100.0, 0.0);
