...
void MainWindow::createRotateDock() {
  ...
  connect(rotateSelectCopyCheck_, &QCheckBox::toggled, this, [this](bool) { updateRotateExtVarFromDock(false); });
...
void MainWindow::showRotateDock() {
  if (!rotateDock_ || !operationManager_ ||
      operationManager_->GetAktiveOperationType() != VOM_ROTATE_OBJECT_ACTIVEPOINT) {
    return;
  }
...
void MainWindow::applyRotateSettings() { updateRotateExtVarFromDock(true); }
...
void MainWindow::refreshRotateDockFromExtVar() {
  ...
  rotatePivotLabel_->setText(QStringLiteral("X: %1  Y: %2").arg(pivot.x, 0, 'f', 2).arg(pivot.y, 0, 'f', 2));
}
