...
bool isRegularStyle(const QString &style) {
  return style.compare(QStringLiteral("Regular"), Qt::CaseInsensitive) == 0 ||
         ...
         style.compare(QStringLiteral("Roman"), Qt::CaseInsensitive) == 0 || style.isEmpty();
}
...
QString systemFontId(const QString &family) { return QStringLiteral("Sys:") + family; }

QString bundleFontId(const QString &family) { return QStringLiteral("Ved:") + family; }
...
unsigned int decodeNextUtf8(std::string_view text, std::size_t &pos) {
  ...
  if (remaining >= 3 && (first & 0xF0U) == 0xE0U && (bytes[pos + 1] & 0xC0U) == 0x80U &&
      (bytes[pos + 2] & 0xC0U) == 0x80U) {
    const unsigned int value = ((first & 0x0FU) << 12U) | ((bytes[pos + 1] & 0x3FU) << 6U) | (bytes[pos + 2] & 0x3FU);
    pos += 3;
    return value;
  }
  if (remaining >= 4 && (first & 0xF8U) == 0xF0U && (bytes[pos + 1] & 0xC0U) == 0x80U &&
      (bytes[pos + 2] & 0xC0U) == 0x80U && (bytes[pos + 3] & 0xC0U) == 0x80U) {
    const unsigned int value = ((first & 0x07U) << 18U) | ((bytes[pos + 1] & 0x3FU) << 12U) |
                               ((bytes[pos + 2] & 0x3FU) << 6U) | (bytes[pos + 3] & 0x3FU);
    pos += 4;
    return value;
  }
  ++pos;
...
bool isRtlCodePoint(unsigned int codePoint) {
  return (codePoint >= 0x0590U && codePoint <= 0x08FFU) || (codePoint >= 0xFB1DU && codePoint <= 0xFDFFU) ||
         (codePoint >= 0xFE70U && codePoint <= 0xFEFFU);
}

bool isStrongLtrCodePoint(unsigned int codePoint) {
  return (codePoint >= 'A' && codePoint <= 'Z') || (codePoint >= 'a' && codePoint <= 'z') ||
         (codePoint >= '0' && codePoint <= '9') || (codePoint >= 0x0400U && codePoint <= 0x052FU) ||
...
TDQtSystemFontProvider::TDQtSystemFontProvider(bool includeSystemFonts) : includeSystemFonts_(includeSystemFonts) {}
...
TDBuiltinVfnFontProvider::TDBuiltinVfnFontProvider(QString fontId, QString displayName, QString resourcePath)
    : fontId_(std::move(fontId)), displayName_(std::move(displayName)), resourcePath_(std::move(resourcePath)) {}

std::vector<TDVecFontDescriptor> TDBuiltinVfnFontProvider::AvailableFonts() const {
  return {{toStdUtf8(fontId_), toStdUtf8(displayName_), true, false}};
}
...
void TDQtSystemFontProvider::EnsureFontIndex() const {
  ...
  for (const QString &fileName :
    ...
    const QByteArray data = file.readAll();
    FT_Face face = nullptr;
    if (FT_New_Memory_Face(freeType.library, reinterpret_cast<const FT_Byte *>(data.constData()),
  ...
  if (includeSystemFonts_) {
    const QStringList filters{QStringLiteral("*.ttf"), QStringLiteral("*.ttc"), QStringLiteral("*.otf"),
                              QStringLiteral("*.otc")};
...
std::vector<TDVecFontDescriptor> TDQtSystemFontProvider::AvailableFonts() const {
  ...
  for (const FontEntry &entry : fontIndex_) {
    fonts.push_back({toStdUtf8(entry.fontId), toStdUtf8(entry.displayName), false, !entry.isResource});
  }
  return fonts;
}
...
std::unique_ptr<TDVecFont> TDQtSystemFontProvider::LoadFont(const std::string &fontId) {
  ...
  for (const FontEntry &entry : fontIndex_) {
    ...
    if (entry.isResource) {
      ...
      const QByteArray data = file.readAll();
      return ved::fontconvert::ConvertTrueTypeMemoryToVecFont(data.constData(), static_cast<long>(data.size()),
    ...
    return ved::fontconvert::ConvertTrueTypeFileToVecFont(
        std::string(encodedPath.constData(), static_cast<std::size_t>(encodedPath.size())), entry.faceIndex,
        toStdUtf8(entry.fontId), ved::fontconvert::CharacterCoverage::FullCmap);
  }
  return nullptr;
}

bool TDQtSystemFontProvider::ShapeText(const TDVecFont *font, const char *utf8Text,
                                       std::vector<TDVecShapedGlyph> &glyphs) const {
  ...
  if (!shapingCache_ || shapingCache_->fontId != requestedFontId) {
    ...
    if (fontEntry->isResource) {
      ...
      if (FT_New_Memory_Face(cache->library, reinterpret_cast<const FT_Byte *>(cache->fontData.constData()),
                             static_cast<FT_Long>(cache->fontData.size()), fontEntry->faceIndex, &cache->face) != 0 ||
          !cache->face) {
        return false;
      }
    } else {
  ...
  while (lineStart <= text.size()) {
    ...
    for (const TextRun &run : splitDirectionalRuns(line)) {
      ...
      hb_buffer_add_utf8(buffer, line.data() + run.start, static_cast<int>(run.length), 0,
                         static_cast<int>(run.length));
