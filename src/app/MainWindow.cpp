...
QString vedFileFilter() { return QStringLiteral("VED documents (*.ved);;All files (*)"); }

QString dialogStartPath(const QString &currentDocumentPath, const QString &lastDocumentDirectory,
...
QString fontWarningText(const VEDFontResolutionResult &fontResolution) {
  ...
  for (const VEDFontResolutionWarning &warning : fontResolution.warnings) {
    ...
    case VEDFontFallbackKind::RequestedMissingUsedArialUnicodeMS:
      lines.push_back(
          QStringLiteral("Font '%1' was not found. The document was opened using '%2'.").arg(requested, resolved));
      break;
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), canvas_(new QVedWidget(this)), selectAction_(nullptr), cutAction_(nullptr),
      copyAction_(nullptr), pasteAction_(nullptr), undoAction_(nullptr), redoAction_(nullptr),
      makeGroupAction_(nullptr), resolveGroupAction_(nullptr), lockObjectAction_(nullptr), moveObjectAction_(nullptr),
      moveNodeAction_(nullptr), insertNodeAction_(nullptr), deleteNodeAction_(nullptr),
      moveBsplineControlPointAction_(nullptr), rotateObjectAction_(nullptr), scaleObjectAction_(nullptr),
      changeEdgeRoundNodeAction_(nullptr), modifyCurveAttributeAction_(nullptr), createTextAction_(nullptr),
      createFrameTextAction_(nullptr), modifyTextAction_(nullptr), zoomAction_(nullptr), panAction_(nullptr),
      showGridAction_(nullptr), gridLockAction_(nullptr), showRulersAction_(nullptr),
      mouseToleranceCrossAction_(nullptr), statusBarSplitter_(nullptr), activeOperationLabel_(nullptr),
      pageFormatLabel_(nullptr), coordinateLabel_(nullptr), curveDock_(nullptr), curveShowPolygonCheck_(nullptr),
      curveShowControlsCheck_(nullptr), curveCopyCheck_(nullptr), curveDegreeLabel_(nullptr), curveDegreeEdit_(nullptr),
      curveResolutionEdit_(nullptr), roundRectangleDock_(nullptr), roundRectangleRadiusEdit_(nullptr),
      roundRectangleResolutionEdit_(nullptr), roundRectangleShowPolygonCheck_(nullptr),
      roundRectangleShowControlsCheck_(nullptr), roundRectangleWidthLabel_(nullptr),
      roundRectangleHeightLabel_(nullptr), rotateDock_(nullptr), rotateAngleEdit_(nullptr), rotateCopyCheck_(nullptr),
      rotateSelectCopyCheck_(nullptr), rotateObjectCountLabel_(nullptr), rotatePivotLabel_(nullptr), textDock_(nullptr),
      textEdit_(nullptr), textXScaleSpin_(nullptr), textYScaleSpin_(nullptr), textAngleSpin_(nullptr),
      textInclineSpin_(nullptr), textLineSpacingSpin_(nullptr), textCharSpacingSpin_(nullptr),
      textResolutionSpin_(nullptr), textFontCombo_(nullptr), textJustificationCombo_(nullptr),
      textVerticalAlignCombo_(nullptr), textVerticalCheck_(nullptr), textUnderlineCheck_(nullptr),
      textScaleDependencyCheck_(nullptr), textDockMode_(TextDockMode::None) {
  resize(1100, 750);
  setStyleSheet(QStringLiteral("QToolButton:checked {"
  ...
  statusBarSplitter_->setStyleSheet(
      QStringLiteral("QSplitter::handle { background: palette(mid); border-radius: 2px; }"));
  ...
  if (!lastDoc.isEmpty() && QFile::exists(lastDoc) && loadDefaultVecFont() && fontManager_) {
    ...
    if (file.open(QIODevice::ReadOnly)) {
      ...
      VEDModelReadResult result =
          LoadVecModelFromBytes(data.constData(), static_cast<std::size_t>(data.size()), *fontManager_);
      if (result.Ok()) {
...
void MainWindow::initializeEditor() {
  ...
  canvas_->setMouseCoordinateCallback([this](TDMatPoint point, bool valid) { updateCoordinateStatus(point, valid); });
}
...
void MainWindow::showFontResolutionWarnings(const VEDFontResolutionResult &fontResolution) {
  ...
  QMessageBox::warning(this, QStringLiteral("Font fallback warning"), message);
}
...
void MainWindow::newDocument() {
  ...
  const TDVecDocumentSettings currentDefaults = model_ ? model_->DocumentSettings() : TDVecDocumentSettings{};
...
void MainWindow::openDocument() {
  ...
  if (!loadDefaultVecFont() || !fontManager_) {
    QMessageBox::warning(this, QStringLiteral("Open VED document"),
                         QStringLiteral("Default font could not be loaded."));
    return;
  }

  const QString fileName = QFileDialog::getOpenFileName(
      this, QStringLiteral("Open VED document"),
      dialogStartPath(currentDocumentPath_, lastDocumentDirectory_, QString()), vedFileFilter());
  if (fileName.isEmpty()) {
    return;
  }
...
bool MainWindow::saveDocument() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, QStringLiteral("Save VED document"),
      dialogStartPath(currentDocumentPath_, lastDocumentDirectory_, QStringLiteral("untitled.ved")), vedFileFilter());
  if (fileName.isEmpty()) {
    return false;
  }
  ...
  if (!result.Ok()) {
    QMessageBox::warning(this, QStringLiteral("Save VED document"),
...
bool MainWindow::confirmDiscardUnsavedChanges() {
  ...
  const QMessageBox::StandardButton result = QMessageBox::warning(
      this, QStringLiteral("Unsaved changes"), QStringLiteral("The current document has unsaved changes."),
...
void MainWindow::createMenus() {
  ...
  convertSystemFontsAction_->setStatusTip(
      QStringLiteral("Scan and convert installed TrueType/OpenType system fonts for use in text objects"));
  ...
  connect(aboutAction, &QAction::triggered, this, [this] {
    QMessageBox::about(this, QStringLiteral("About VED Qt Port"),
...
bool MainWindow::cancelDialogOperationFromRightClick() {
  if (operationManager_) {
    ...
    if (activeOperation == VOC_VECTEXT || activeOperation == VOC_VECFRAMETEXT_EMPTY ||
        activeOperation == VOC_POLYCURVE || activeOperation == VOC_BSPLINE_CONTROLPOINT) {
      return false;
    }
  }
  ...
  const TDViewOperationEnum activeOperation = operationManager_->GetAktiveOperationType();
  if (curveDock_ && curveDock_->isVisible() && activeOperation == VOM_MODIFY_CURVE_ATTRIBUTE) {
    cancelCurveDockOperation();
    return true;
  }

  if (roundRectangleDock_ && roundRectangleDock_->isVisible() && activeOperation == VOC_ROUND_RECTANGLE_NOTROTATED) {
    cancelRoundRectangleDockOperation();
    return true;
  }

  if (rotateDock_ && rotateDock_->isVisible() && activeOperation == VOM_ROTATE_OBJECT_ACTIVEPOINT) {
...
void MainWindow::onToggleSystemFonts(bool enabled) {
  ...
  statusBar()->showMessage(enabled ? QStringLiteral("System fonts enabled — installed fonts converted")
...
void MainWindow::updateModifyCommandActions() {
  ...
  if (selectedObject) {
    canChangeEdgeRoundNode =
        selectedObject->GetType() == VECOBJ_POLYCURVE || selectedObject->GetType() == VECOBJ_POLYCURVEAREA;
    canModifyCurveAttribute =
        selectedObject->GetType() == VECOBJ_BSPLINE || selectedObject->GetType() == VECOBJ_POLYCURVE;
    canModifyText = selectedObject->GetType() == VECOBJ_TEX || selectedObject->GetType() == VECOBJ_FRAMETEXT;
  }
  ...
  if (lockObjectAction_) {
    ...
    lockObjectAction_->setChecked(hasAnySelection && (model_->GetLockResizeForSelectedObjects() != 0 ||
  ...
  if (modifyCurveAttributeAction_) {
    ...
    if (!canModifyCurveAttribute && curveDock_ && operationManager_ &&
...
void MainWindow::updateMouseToleranceCrossCursor() {
  ...
  const TDViewOperationEnum activeOperation = operationManager_->GetAktiveOperationType();
  const bool selectOperation =
      activeOperation == VOM_SELECT_MOVE_NODE_OBJECT || activeOperation == VOM_SELECTMOVE_OBJ_SCALE_FRAME;
  if (!selectOperation) {
    return;
  }
...
void MainWindow::updateCoordinateStatus(TDMatPoint point, bool valid) {
  ...
  const TDVecUnitSettings unitSettings = model_ ? model_->UnitSettings() : TDVecUnitSettings{};
  const TDVecUnitFormatter formatter(unitSettings);
  coordinateLabel_->setText(QStringLiteral("X: %1  Y: %2")
...
void MainWindow::exportPdf() {
  ...
  exportEngine.SetWorkSpaceRange(ps.pageOriginX, ps.pageOriginY, ps.pageOriginX + ps.widthReal,
                                 ps.pageOriginY + ps.heightReal);
  exportEngine.SetWorldSpaceRange(ps.pageOriginX, ps.pageOriginY, ps.pageOriginX + ps.widthReal,
                                  ps.pageOriginY + ps.heightReal);
  exportEngine.SetViewRange(ps.pageOriginX, ps.pageOriginY, ps.pageOriginX + ps.widthReal,
                            ps.pageOriginY + ps.heightReal);
  ...
  painter.end();
  statusBar()->showMessage(QStringLiteral("PDF exported: %1").arg(QFileInfo(fileName).fileName()), 2500);
}
...
void MainWindow::printDocument() {
  ...
  if (!painter.isActive()) {
    QMessageBox::warning(this, QStringLiteral("Print"), QStringLiteral("Could not start printing."));
    return;
  }
  ...
  printEngine.SetWorkSpaceRange(ps.pageOriginX, ps.pageOriginY, ps.pageOriginX + ps.widthReal,
                                ps.pageOriginY + ps.heightReal);
  printEngine.SetWorldSpaceRange(ps.pageOriginX, ps.pageOriginY, ps.pageOriginX + ps.widthReal,
                                 ps.pageOriginY + ps.heightReal);
  printEngine.SetViewRange(ps.pageOriginX, ps.pageOriginY, ps.pageOriginX + ps.widthReal,
                           ps.pageOriginY + ps.heightReal);
...
void MainWindow::updatePageFormatStatus() {
  ...
  const QString orientation =
      ps.orientation == TDVecPageOrientation::Portrait ? QStringLiteral("Portrait") : QStringLiteral("Landscape");
  ...
  pageFormatLabel_->setText(QStringLiteral("%1 %2 — %3 × %4")
...
void MainWindow::updateWindowTitle() {
  if (currentDocumentPath_.isEmpty()) {
    ...
    setWindowTitle(QStringLiteral("%1 — VED Qt Port").arg(QFileInfo(currentDocumentPath_).fileName()));
  }
}
