...
bool forwardsOperationMouseEvents(QVedWidget::InteractionMode mode) {
  return mode == QVedWidget::InteractionMode::CreateLine || mode == QVedWidget::InteractionMode::CreateText ||
         mode == QVedWidget::InteractionMode::SelectObject || mode == QVedWidget::InteractionMode::MoveObject ||
         mode == QVedWidget::InteractionMode::DeleteObject || mode == QVedWidget::InteractionMode::ModifyObject;
}
} // namespace

QVedWidget::QVedWidget(QWidget *parent)
    : QWidget(parent), horizontalScrollBar_(new QScrollBar(Qt::Horizontal, this)),
      verticalScrollBar_(new QScrollBar(Qt::Vertical, this)), interactionMode_(InteractionMode::None),
      activeCursor_(VECVIEW_CURSOR_DEFAULT), snapMousePoint_{0.0, 0.0}, selectionAreaStart_{0.0, 0.0},
      selectionAreaCurrent_{0.0, 0.0}, documentSettings_(nullptr), showPageBounds_(true), showGrid_(false),
      showRulers_(false), gridLock_(false), showMouseToleranceCross_(false), zoomToolEnabled_(false),
      panToolEnabled_(false), viewInitialized_(false), panning_(false), updatingScrollBars_(false),
      hasSnapMousePoint_(false), selectionAreaActive_(false) {
  ...
  connect(horizontalScrollBar_, &QScrollBar::valueChanged, this, [this](int) { setPanFromScrollBars(); });
  connect(verticalScrollBar_, &QScrollBar::valueChanged, this, [this](int) { setPanFromScrollBars(); });
...
TDVecViewInterfaceBase *QVedWidget::viewInterface() { return &viewInterface_; }

TDGraphicEngineQt *QVedWidget::graphicEngine() { return &graphicEngine_; }
...
QVedWidget::InteractionMode QVedWidget::interactionMode() const { return interactionMode_; }

void QVedWidget::setDocumentSettings(const TDVecDocumentSettings *settings) { documentSettings_ = settings; }
...
bool QVedWidget::showGrid() const { return showGrid_; }
...
bool QVedWidget::showRulers() const { return showRulers_; }
...
bool QVedWidget::gridLock() const { return gridLock_; }
...
bool QVedWidget::showMouseToleranceCross() const { return showMouseToleranceCross_; }
...
bool QVedWidget::zoomToolEnabled() const { return zoomToolEnabled_; }
...
bool QVedWidget::panToolEnabled() const { return panToolEnabled_; }
...
VEDDocumentViewState QVedWidget::documentViewState() {
  ...
  viewState.center = {(viewRange.Left + viewRange.Right) / 2.0, (viewRange.Top + viewRange.Bottom) / 2.0};
...
void QVedWidget::applyDocumentViewState(const VEDDocumentViewState &viewState) {
  ...
  const double zoom = std::isfinite(viewState.zoom) ? std::clamp(viewState.zoom, 0.02, 80.0) : 1.0;
  ...
  graphicEngine_.SetWorldSpaceRange(std::min(worldRange.Left, left), std::min(worldRange.Top, top),
                                    std::max(worldRange.Right, right), std::max(worldRange.Bottom, bottom));
...
void QVedWidget::paintEvent(QPaintEvent *event) {
  ...
  graphicEngine_.SetDeviceMetrics(viewRect.width(), viewRect.height(), painter.device()->logicalDpiX(),
...
void QVedWidget::mousePressEvent(QMouseEvent *event) {
  ...
  if (!panToolEnabled_ && forwardsOperationMouseEvents(interactionMode_)) {
    ...
    if (operationMouseDownCallback_) {
      operationMouseDownCallback_(virtualButton(event->button()), virtualKeyState(event->modifiers()),
                                  operationPoint(event->pos()));
      event->accept();
      return;
    }
  }
...
void QVedWidget::mouseDoubleClickEvent(QMouseEvent *event) { mousePressEvent(event); }
...
void QVedWidget::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (forwardsOperationMouseEvents(interactionMode_) && canvasRect().contains(event->pos())) {
    ...
    if (operationMouseMoveCallback_) {
      operationMouseMoveCallback_(virtualButtons(event->buttons()), virtualKeyState(event->modifiers()),
                                  operationPoint(event->pos()));
    }
    event->accept();
    return;
  ...
  if (panning_) {
    ...
    graphicEngine_.SetPanAllowingWorldExpansion(graphicEngine_.GetPanX() - diff.x(),
...
void QVedWidget::mouseReleaseEvent(QMouseEvent *event) {
  ...
  if (forwardsOperationMouseEvents(interactionMode_) && canvasRect().contains(event->pos())) {
    ...
    if (operationMouseUpCallback_) {
      operationMouseUpCallback_(virtualButton(event->button()), virtualKeyState(event->modifiers()),
                                operationPoint(event->pos()));
    }
    event->accept();
    return;
...
void QVedWidget::keyPressEvent(QKeyEvent *event) { QWidget::keyPressEvent(event); }

void QVedWidget::keyReleaseEvent(QKeyEvent *event) { QWidget::keyReleaseEvent(event); }

void QVedWidget::requestViewRefresh() { update(); }
...
void QVedWidget::refreshDeviceMetrics() {
  ...
  graphicEngine_.SetDeviceMetrics(viewRect.width(), viewRect.height(), logicalDpiX(), logicalDpiY());
}
...
void QVedWidget::initializeViewIfNeeded() {
  ...
  graphicEngine_.SetWorldSpaceRange(originX - kDocumentMargin, originY - kDocumentMargin,
                                    originX + pageWidth + kDocumentMargin, originY + pageHeight + kDocumentMargin);
  graphicEngine_.SetViewRange(originX - kDocumentMargin, originY - kDocumentMargin,
                              originX + pageWidth + kDocumentMargin, originY + pageHeight + kDocumentMargin);
  viewInitialized_ = true;
  updateScrollBars();
}
...
void QVedWidget::updateScrollBars() {
  ...
  const int totalWidth = std::max(
      viewRect.width(), static_cast<int>(MatRound(std::fabs((worldRange.Right - worldRange.Left) / realPerPixelX))));
  const int totalHeight = std::max(
      viewRect.height(), static_cast<int>(MatRound(std::fabs((worldRange.Bottom - worldRange.Top) / realPerPixelY))));
...
void QVedWidget::handleHorizontalScrollAction(int action) {
  ...
  horizontalScrollBar_->setSliderPosition(
      std::clamp(value, horizontalScrollBar_->minimum(), horizontalScrollBar_->maximum()));
}
...
void QVedWidget::handleVerticalScrollAction(int action) {
  ...
  verticalScrollBar_->setSliderPosition(
      std::clamp(value, verticalScrollBar_->minimum(), verticalScrollBar_->maximum()));
}

int QVedWidget::horizontalLineStep() const { return std::max(1, horizontalPageScrollStep() / kScrollsPerPage); }

int QVedWidget::verticalLineStep() const { return std::max(1, verticalPageScrollStep() / kScrollsPerPage); }
...
void QVedWidget::drawGrid() {
  ...
  const double gridMajorStep =
      documentSettings_ ? documentSettings_->gridSettings.majorStepReal : kDefaultGridMajorStep;
  const int gridSubdivisions =
      documentSettings_ ? documentSettings_->gridSettings.subdivisions : kDefaultGridSubdivisions;
  const long gridResLimit =
      documentSettings_ ? documentSettings_->gridSettings.resolutionLimitPixels : kDefaultGridResolutionLimit;

  const double minStepByPixel = std::max(std::fabs(graphicEngine_.ScreenToXVal(gridResLimit)),
  ...
  const double minorFromPreferred =
      (gridSubdivisions > 0) ? gridMajorStep / static_cast<double>(gridSubdivisions) : gridMajorStep;
  const double minimumStep = std::max({minorFromPreferred, minStepByPixel, minStepByPointBudget});
  ...
  if (drawSubGrid) {
    ...
    for (long y = firstY; y <= lastY; y += gridStep) {
      for (long x = firstX; x <= lastX; x += gridStep) {
        ...
        const TDMatLine pointLine{static_cast<double>(x), static_cast<double>(y), static_cast<double>(x),
  ...
  for (long y = firstY; y <= lastY; y += gridStep) {
    for (long x = firstX; x <= lastX; x += gridStep) {
      ...
      const TDMatLine horizontalLine{static_cast<double>(x) - mainMarkerHalfWidth, static_cast<double>(y),
                                     static_cast<double>(x) + mainMarkerHalfWidth, static_cast<double>(y)};
      const TDMatLine verticalLine{static_cast<double>(x), static_cast<double>(y) - mainMarkerHalfHeight,
                                   static_cast<double>(x), static_cast<double>(y) + mainMarkerHalfHeight};
...
void QVedWidget::drawRulers() {
  ...
  const TDVecUnitSettings unitSettings = documentSettings_ ? documentSettings_->unitSettings : TDVecUnitSettings{};
  const double gridMajorStep =
      documentSettings_ ? documentSettings_->gridSettings.majorStepReal : kDefaultGridMajorStep;
  const int gridSubdivisions =
      documentSettings_ ? documentSettings_->gridSettings.subdivisions : kDefaultGridSubdivisions;
  ...
  const TDVecMeasureScale scale =
      calculator.CalculateForView(realPerPixel, gridResLimit, gridMajorStep, gridSubdivisions);
...
double QVedWidget::currentZoom() const { return graphicEngine_.GetZoom(); }
...
bool QVedWidget::isRectVisible(TDMatRect rect) const { return graphicEngine_.IsRectVisible(rect); }

long QVedWidget::mouseTolerance() const { return graphicEngine_.GetMouseTolerance(); }

double QVedWidget::minimumDistance() const { return graphicEngine_.GetMinimumDistance(); }

void QVedWidget::zoomInOutEvent(TDVOPZoomInOutExtVar *extVar) const { Q_UNUSED(extVar); }

TDMatPoint QVedWidget::screenToRealPoint(const QPoint &screenPoint) const {
  return {graphicEngine_.ScreenToXPos(screenPoint.x()), graphicEngine_.ScreenToYPos(screenPoint.y())};
}
...
void QVedWidget::updateSnapMousePoint(const QPoint &screenPoint) {
  if (!canvasRect().contains(screenPoint) || (!shouldDrawMouseToleranceMarker() && !shouldDrawMouseToleranceCross())) {
    if (hasSnapMousePoint_) {
      hasSnapMousePoint_ = false;
      update();
    }
...
bool QVedWidget::shouldDrawMouseToleranceMarker() const {
  return activeCursor_ == VECVIEW_SELECT_OBJECT || interactionMode_ == InteractionMode::SelectObject;
}
...
bool QVedWidget::shouldDrawMouseToleranceCross() const {
  ...
  return interactionMode_ == InteractionMode::CreateLine || interactionMode_ == InteractionMode::CreateText ||
         interactionMode_ == InteractionMode::MoveObject || interactionMode_ == InteractionMode::DeleteObject ||
