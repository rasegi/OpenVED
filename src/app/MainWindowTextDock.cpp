...
public:
  explicit FontComboBox(QWidget *parent = nullptr) : QComboBox(parent) { setFocusPolicy(Qt::StrongFocus); }
...
private:
  ...
  void selectSearchIndex(int index) {
    ...
    QTimer::singleShot(1200, this, [this] { searchText_.clear(); });
  }
...
void MainWindow::showTextDockForModify() {
  ...
  auto *extVar =
      operationManager_ ? dynamic_cast<TDVOMVecTextExtVar *>(operationManager_->GetActiveVOPExtVariables()) : nullptr;
  if (!extVar) {
    return;
  }
...
void MainWindow::cancelTextDockOperation() {
  ...
  if (previousMode == TextDockMode::CreateText || previousMode == TextDockMode::CreateFrameTextBox ||
      (previousMode == TextDockMode::Modify && operationManager_ &&
...
bool MainWindow::rebuildFontProviders() {
  ...
  fontProviders_.push_back(std::make_unique<TDBuiltinVfnFontProvider>(
      QStringLiteral("Ved:WPS Default"), QStringLiteral("WPS Default"), QStringLiteral(":/ved/font/wps_default.vfn")));
...
void MainWindow::populateTextFontCombo(bool force) {
  ...
  const int selectedIndex = textFontCombo_->findData(
      currentFontId.isEmpty() ? QStringLiteral("Ved:WPS Default") : currentFontId, kFontIdRole);
...
const TDVecFont *MainWindow::currentTextVecFont() {
  ...
  const QString fontId =
      textFontCombo_ ? textFontCombo_->currentData(kFontIdRole).toString() : QStringLiteral("Ved:WPS Default");
