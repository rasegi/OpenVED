...
TDVecPageSettings pageSettingsForFormat(int index, TDVecPageOrientation orientation) {
  switch (index) {
  case kFormatA4:
    return TDVecPageFormats::A4(orientation);
  case kFormatA3:
    return TDVecPageFormats::A3(orientation);
  case kFormatA5:
    return TDVecPageFormats::A5(orientation);
  case kFormatLetter:
    return TDVecPageFormats::Letter(orientation);
  default:
    return {};
  }
}
...
DocumentSetupDialog::DocumentSetupDialog(const TDVecDocumentSettings &currentSettings, QWidget *parent)
    : QDialog(parent), unitSettings_(currentSettings.unitSettings) {
  ...
  unitCombo_->addItems(
      {QStringLiteral("Millimeter (mm)"), QStringLiteral("Centimeter (cm)"), QStringLiteral("Inch (in)")});
  topForm->addRow(QStringLiteral("Unit:"), unitCombo_);

  formatCombo_ = new QComboBox(this);
  formatCombo_->addItems({QStringLiteral("A4"), QStringLiteral("A3"), QStringLiteral("A5"), QStringLiteral("Letter"),
                          QStringLiteral("Custom")});
  topForm->addRow(QStringLiteral("Format:"), formatCombo_);

  orientationCombo_ = new QComboBox(this);
  orientationCombo_->addItems({QStringLiteral("Portrait"), QStringLiteral("Landscape")});
  ...
  orientationCombo_->setCurrentIndex(currentSettings.pageSettings.orientation == TDVecPageOrientation::Landscape ? 1
                                                                                                                 : 0);
  ...
  connect(unitCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &DocumentSetupDialog::onUnitChanged);
  connect(formatCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &DocumentSetupDialog::onFormatChanged);
  connect(orientationCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &DocumentSetupDialog::onOrientationChanged);
}
...
TDVecDocumentSettings DocumentSetupDialog::documentSettings() const {
  ...
  case kFormatA5:
    ...
    break;
  case kFormatLetter:
    ps.formatName = "Letter";
    break;
  default:
    ps.formatName = "Custom";
    break;
  }
  ps.orientation =
      orientationCombo_->currentIndex() == 1 ? TDVecPageOrientation::Landscape : TDVecPageOrientation::Portrait;
...
void DocumentSetupDialog::onFormatChanged(int index) {
  ...
  const TDVecPageOrientation orientation =
      orientationCombo_->currentIndex() == 1 ? TDVecPageOrientation::Landscape : TDVecPageOrientation::Portrait;
...
void DocumentSetupDialog::onOrientationChanged(int index) {
  ...
  if (formatCombo_->currentIndex() != kFormatCustom) {
    const TDVecPageOrientation orientation =
        index == 1 ? TDVecPageOrientation::Landscape : TDVecPageOrientation::Portrait;
    const TDVecPageSettings preset = pageSettingsForFormat(formatCombo_->currentIndex(), orientation);
    widthSpin_->setValue(toDisplay(preset.widthReal));
    heightSpin_->setValue(toDisplay(preset.heightReal));
    return;
  }
...
void DocumentSetupDialog::updateDimensionReadOnly() {
  ...
  widthSpin_->setButtonSymbols(isCustom ? QAbstractSpinBox::UpDownArrows : QAbstractSpinBox::NoButtons);
  heightSpin_->setButtonSymbols(isCustom ? QAbstractSpinBox::UpDownArrows : QAbstractSpinBox::NoButtons);
}
...
TDVecDisplayUnit DocumentSetupDialog::selectedUnit() const {
  ...
  case kUnitCentimeter:
    return TDVecDisplayUnit::Centimeter;
  case kUnitInch:
    return TDVecDisplayUnit::Inch;
  default:
    return TDVecDisplayUnit::Millimeter;
  }
}
