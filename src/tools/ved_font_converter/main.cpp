...
void printUsage(const char *prog) {
  std::cerr << "Usage: " << prog << " --in <font.ttf|otf> --out <font.vfn> --name <font-name>\n"
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    std::string value;
    if (arg == "--in") {
      if (!takeValue(argc, argv, i, inPath)) {
        std::cerr << "error: --in requires a value\n";
        return 2;
      }
    } else if (arg == "--out") {
      if (!takeValue(argc, argv, i, outPath)) {
        std::cerr << "error: --out requires a value\n";
        return 2;
      }
    } else if (arg == "--name") {
      if (!takeValue(argc, argv, i, name)) {
        std::cerr << "error: --name requires a value\n";
        return 2;
      }
    } else if (arg == "--face") {
      if (!takeValue(argc, argv, i, value)) {
        std::cerr << "error: --face requires a value\n";
        return 2;
      }
      faceIndex = std::strtol(value.c_str(), nullptr, 10);
    } else if (arg == "--header") {
      if (!takeValue(argc, argv, i, value)) {
        std::cerr << "error: --header requires a value\n";
        return 2;
      }
  ...
  std::unique_ptr<TDVecFont> font = ved::fontconvert::ConvertTrueTypeFileToVecFont(
      inPath, faceIndex, name, ved::fontconvert::CharacterCoverage::FullCmap);
  if (!font) {
    std::cerr << "error: failed to load or convert '" << inPath << "' (face " << faceIndex << ")\n";
    return 1;
  }
  ...
  std::cout << "ok: " << inPath << " -> " << outPath << "  (" << font->CountVecCharacters() << " chars, "
