...
template <class T> T *insertOwned(std::vector<std::unique_ptr<T>> &items, int index, T *object) {
...
template <class T> T *appendOwned(std::vector<std::unique_ptr<T>> &items, T *object) {
  items.push_back(std::unique_ptr<T>(object));
  return object;
}

template <class T> bool removeOwned(std::vector<std::unique_ptr<T>> &items, int index) {
  if (index < 0 || index >= static_cast<int>(items.size())) {
    return false;
  }
...
template <class T> bool deleteOwned(std::vector<std::unique_ptr<T>> &items, int index) {
  if (index < 0 || index >= static_cast<int>(items.size())) {
    return false;
  }
  items[index].reset();
...
template <class T> T *dropOwned(std::vector<std::unique_ptr<T>> &items, int index) {
  if (index < 0 || index >= static_cast<int>(items.size())) {
    return nullptr;
  }
...
template <class T> T *getOwned(const std::vector<std::unique_ptr<T>> &items, int index) {
  if (index < 0 || index >= static_cast<int>(items.size())) {
    return nullptr;
  }
...
template <class T> void compactOwned(std::vector<std::unique_ptr<T>> &items) {
  items.erase(
      std::remove_if(items.begin(), items.end(), [](const std::unique_ptr<T> &item) { return item == nullptr; }),
      items.end());
}

template <class T> void writeObjectVector(VEDBinaryWriter &writer, const std::vector<std::unique_ptr<T>> &items) {
...
template <class T> void readObjectVector(VEDBinaryReader &reader, std::vector<std::unique_ptr<T>> &items) {
  items.clear();
...
void expandFrame(TDMatRect &frame, const TDMatRect &next, bool &initialized) {
  ...
  const double left =
      std::min({frame.P1.x, frame.P2.x, frame.P3.x, frame.P4.x, next.P1.x, next.P2.x, next.P3.x, next.P4.x});
  const double right =
      std::max({frame.P1.x, frame.P2.x, frame.P3.x, frame.P4.x, next.P1.x, next.P2.x, next.P3.x, next.P4.x});
  const double top =
      std::min({frame.P1.y, frame.P2.y, frame.P3.y, frame.P4.y, next.P1.y, next.P2.y, next.P3.y, next.P4.y});
  const double bottom =
      std::max({frame.P1.y, frame.P2.y, frame.P3.y, frame.P4.y, next.P1.y, next.P2.y, next.P3.y, next.P4.y});
  frame.P1 = {left, top};
  frame.P2 = {right, top};
...
TDVecGlyph::TDVecGlyph() : mnCharWidth(0.0) { mBaseLine.InitialWithNULL(); }

TDVecGlyph::TDVecGlyph(const TDVecGlyph &glyph) : mBaseLine(glyph.mBaseLine), mnCharWidth(glyph.mnCharWidth) {
...
TDVecGlyph *TDVecGlyph::Clone() const { return new TDVecGlyph(*this); }

std::uint32_t TDVecGlyph::StreamFourCC() { return VEDMakeFourCC('v', 'g', 'l', 'y'); }
...
bool TDVecGlyph::IsSpacing() const { return polyCurves_.empty(); }
...
TDMatLine TDVecGlyph::GetBaseLine() const { return mBaseLine; }

void TDVecGlyph::SetCharWidth(double nCharWidth) { mnCharWidth = nCharWidth; }

double TDVecGlyph::GetCharWidth() const { return mnCharWidth; }

TDVecPolyCurve *TDVecGlyph::InsertPolyCurve(int i, TDVecPolyCurve *pObject) {
  return insertOwned(polyCurves_, i, pObject);
}
...
bool TDVecGlyph::ClearPolyCurves() {
  polyCurves_.clear();
  return true;
}
...
TDVecCharacter::TDVecCharacter() : mpVecGlyph(std::make_unique<TDVecGlyph>()), miUnicodeValue(0) {}
...
std::uint32_t TDVecCharacter::StreamFourCC() { return VEDMakeFourCC('v', 'c', 'h', 'r'); }
...
void TDVecCharacter::SetBaseLine(double nAscent) {
  if (mpVecGlyph)
    mpVecGlyph->SetBaseLine(nAscent);
}
TDMatLine TDVecCharacter::GetBaseLine() const { return mpVecGlyph ? mpVecGlyph->GetBaseLine() : TDMatLine{}; }
void TDVecCharacter::SetCharWidth(double nCharWidth) {
  if (mpVecGlyph)
    mpVecGlyph->SetCharWidth(nCharWidth);
}
double TDVecCharacter::GetCharWidth() const { return mpVecGlyph ? mpVecGlyph->GetCharWidth() : 0.0; }
TDVecPolyCurve *TDVecCharacter::InsertPolyCurve(int i, TDVecPolyCurve *pObject) {
  return mpVecGlyph ? mpVecGlyph->InsertPolyCurve(i, pObject) : nullptr;
}
TDVecPolyCurve *TDVecCharacter::AppendPolyCurve(TDVecPolyCurve *pObject) {
  return mpVecGlyph ? mpVecGlyph->AppendPolyCurve(pObject) : nullptr;
}
bool TDVecCharacter::RemovePolyCurve(int iObject) { return mpVecGlyph && mpVecGlyph->RemovePolyCurve(iObject); }
TDVecPolyCurve *TDVecCharacter::DropPolyCurve(int iObject) {
  return mpVecGlyph ? mpVecGlyph->DropPolyCurve(iObject) : nullptr;
}
...
TDVecPolyCurve *TDVecCharacter::GetPolyCurve(int iObject) const {
  return mpVecGlyph ? mpVecGlyph->GetPolyCurve(iObject) : nullptr;
}
int TDVecCharacter::CountPolyCurves() const { return mpVecGlyph ? mpVecGlyph->CountPolyCurves() : 0; }
void TDVecCharacter::CompactPolyCurves() {
  if (mpVecGlyph)
    mpVecGlyph->CompactPolyCurves();
}

TDVecFont::TDVecFont() : mnHeight(0.0), mnAscent(0.0), mnSpacingGlyphWidth(0.0), msFontName{} {}

TDVecFont::TDVecFont(const TDVecFont &font)
    : mnHeight(font.mnHeight), mnAscent(font.mnAscent), mnSpacingGlyphWidth(font.mnSpacingGlyphWidth), msFontName{} {
...
std::uint32_t TDVecFont::StreamFourCC() { return VEDMakeFourCC('v', 'f', 'n', 't'); }
...
TDVecCharacter *TDVecFont::InsertVecCharacter(int i, TDVecCharacter *pObject) {
  return insertOwned(characters_, i, pObject);
}
...
bool TDVecFont::ClearVecCharacters() {
  characters_.clear();
  return true;
}
