...
TDVecEditCad::TDVecEditCad()
    : mpActiveViewInterface(nullptr), mpViewOperationManager(nullptr), mpTmpObject(nullptr), mbTmpObjectOutLine(false),
      mbVisualNodes(true), mbVisualFrame(true), meUsedCursor(VECVIEW_CURSOR_DEFAULT) {
...
void TDVecEditCad::MouseDown(TDVecViewInterfaceBase *pVecViewInterface, TDOPVirtMouseButton Button,
                             TDOPVirtKeyState Shift, double X, double Y) {
...
void TDVecEditCad::MouseUp(TDVecViewInterfaceBase *pVecViewInterface, TDOPVirtMouseButton Button,
                           TDOPVirtKeyState Shift, double X, double Y) {
  mpActiveViewInterface = pVecViewInterface;
  if (mpViewOperationManager) {
...
void TDVecEditCad::MouseMove(TDVecViewInterfaceBase *pVecViewInterface, TDOPVirtMouseButton Button,
                             TDOPVirtKeyState Shift, double X, double Y) {
  mpActiveViewInterface = pVecViewInterface;
  if (mpViewOperationManager) {
...
TDVecViewInterfaceBase *TDVecEditCad::GetActiveView() { return mpActiveViewInterface; }
...
void TDVecEditCad::SetVisualNodes(bool bVisualNodes) { mbVisualNodes = bVisualNodes; }

void TDVecEditCad::SetVisualFrame(bool bVisualFrame) { mbVisualFrame = bVisualFrame; }

bool TDVecEditCad::GetVisualNodes() { return mbVisualNodes; }

bool TDVecEditCad::GetVisualFrame() { return mbVisualFrame; }
...
TDViewOperationManager *TDVecEditCad::GetViewOperationManager() { return mpViewOperationManager; }
...
TDVecViewCursor TDVecEditCad::GetUsedCursor() const { return meUsedCursor; }
