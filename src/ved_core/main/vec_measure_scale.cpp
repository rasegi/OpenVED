...
TDVecMeasureScaleCalculator::TDVecMeasureScaleCalculator(TDVecUnitSettings unitSettings) : formatter_(unitSettings) {}

TDVecMeasureScale TDVecMeasureScaleCalculator::CalculateForView(double realPerPixel, long minimumPixelDistance,
                                                                double preferredMajorStepReal, int subdivisions) const {
  ...
  const double minorFromPreferred =
      (subdivisions > 0) ? preferredMajorStepReal / static_cast<double>(subdivisions) : preferredMajorStepReal;
...
std::vector<TDVecMeasureTick> TDVecMeasureScaleCalculator::BuildTicks(double visibleMinReal, double visibleMaxReal,
