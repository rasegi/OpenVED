...
bool isFrameInBounds(TDMatRect frame, double left, double top, double right, double bottom) {
  return isPointInBounds(frame.P1, left, top, right, bottom) && isPointInBounds(frame.P2, left, top, right, bottom) &&
         isPointInBounds(frame.P3, left, top, right, bottom) && isPointInBounds(frame.P4, left, top, right, bottom);
}
...
bool TDVecModelHitResult::IsHit() const { return objectIndex >= 0 && object != nullptr && lineHit.IsHit(); }

bool TDVecModelHitResult::IsNodeHit() const { return IsHit() && lineHit.IsNodeHit(); }

TDVecModel::TDVecModel()
    : topLeftArea_{0.0, 0.0}, bottomRightArea_{210000.0, 297000.0}, defaultColor_(0x00000000), documentSettings_{},
      changed_(false), revision_(0) {}

TDVecModel::~TDVecModel() = default;

TDMatPoint TDVecModel::GetTopLeftArea() const { return topLeftArea_; }

TDMatPoint TDVecModel::GetBottomRightArea() const { return bottomRightArea_; }
...
TDRgbColor TDVecModel::GetDefaultColor() const { return defaultColor_; }

bool TDVecModel::IsChanged() const { return changed_; }

void TDVecModel::SetChanged(bool changed) { changed_ = changed; }
...
std::uint64_t TDVecModel::Revision() const { return revision_; }

const TDVecDocumentSettings &TDVecModel::DocumentSettings() const { return documentSettings_; }

const TDVecUnitSettings &TDVecModel::UnitSettings() const { return documentSettings_.unitSettings; }

const TDVecGridSettings &TDVecModel::GridSettings() const { return documentSettings_.gridSettings; }

const TDVecPageSettings &TDVecModel::PageSettings() const { return documentSettings_.pageSettings; }
...
int TDVecModel::CountObjects() const { return static_cast<int>(objects_.size()); }
