...
void SwapReal(double &a, double &b) { std::swap(a, b); }
...
double MatDistance2Point(TDMatPoint p1, TDMatPoint p2) { return std::hypot(p1.x - p2.x, p1.y - p2.y); }
...
bool MatCircleKanteReal(TDMatPoint p1, TDMatPoint p2, TDMatPoint p3, TDMatCircle *circle) {
  ...
  radius = std::sqrt(std::pow(static_cast<long double>(x - p1.x), 2) + std::pow(static_cast<long double>(y - p1.y), 2));
