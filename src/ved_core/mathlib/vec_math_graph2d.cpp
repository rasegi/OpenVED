...
const double MAX_REAL_VALUE =
    std::sqrt(std::sqrt(std::numeric_limits<double>::max())); // vierte Wurzel aus der größtmöglichen double-Zahl
const double MIN_REAL_VALUE = -MAX_REAL_VALUE;                // ist gleich -MAX_REAL_VALUE
const double MIN_POSITIV_REAL_VALUE =
    (1 / MAX_REAL_VALUE); // Wenn ein positiver Wert diesen unterschreitet, wird sie als 0.0 interpretiert.
const double MAX_NEGATIV_REAL_VALUE =
    -(1 / MAX_REAL_VALUE); // Wenn ein negativer Wert diesen  überschreitet, wird sie als 0.0 interpretiert.
...
const TDCoordinateSystem2D MAX_RANGE_2D = // Definiert den größtmöglichen 2D-Vektorraum
    {MIN_REAL_VALUE, MAX_REAL_VALUE, MIN_REAL_VALUE, MAX_REAL_VALUE};
