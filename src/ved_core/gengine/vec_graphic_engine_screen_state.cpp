...
TDGraphicEngineScreenState::TDGraphicEngineScreenState()
    : clientSize_(DEFAULT_CLIENTSIZE), realViewRange_{0.0, 0.0, 0.0, 0.0}, realWorldSpaceRange_{0.0, 0.0, 0.0, 0.0},
      realWorkSpaceRange_{0.0, 0.0, 0.0, 0.0}, realPerPixelX_(0.0), realPerPixelY_(0.0), zoom_(1.0),
      zoomFraction_{1.0, 1.0}, inchPerReal_(INCH_PER_MICROMETER), dpiX_(96), dpiY_(96), mouseTolerance_(5),
      minDistPixels_(2), marginX_(0.0), marginY_(0.0), panX_(0), panY_(0) {
...
long TDGraphicEngineScreenState::RealToXVal(double x) const { return MatRound(x / std::fabs(realPerPixelX_)); }

long TDGraphicEngineScreenState::RealToYVal(double y) const { return MatRound(y / std::fabs(realPerPixelY_)); }
...
void TDGraphicEngineScreenState::SetPanAllowingWorldExpansion(long nPanX, long nPanY) {
  ...
  marginX_ = std::max(std::fabs(realWorldSpaceRange_.Left - realWorkSpaceRange_.Left),
                      std::fabs(realWorldSpaceRange_.Right - realWorkSpaceRange_.Right));
  marginY_ = std::max(std::fabs(realWorldSpaceRange_.Top - realWorkSpaceRange_.Top),
...
long TDGraphicEngineScreenState::GetPanX() const { return panX_; }

long TDGraphicEngineScreenState::GetPanY() const { return panY_; }

double TDGraphicEngineScreenState::GetMarginX() const { return marginX_; }

double TDGraphicEngineScreenState::GetMarginY() const { return marginY_; }
...
long TDGraphicEngineScreenState::GetMouseTolerance() const { return mouseTolerance_; }
...
void TDGraphicEngineScreenState::SetInchPerReal(double nNewInchPerReal) { inchPerReal_ = nNewInchPerReal; }

double TDGraphicEngineScreenState::GetInchPerReal() const { return inchPerReal_; }
...
double TDGraphicEngineScreenState::GetZoom() const { return zoom_; }
...
void TDGraphicEngineScreenState::SetViewSize(TDRecSize recSize) { SetViewSize(recSize.Width, recSize.Height); }
...
const TDCoordinateSystem2D &TDGraphicEngineScreenState::GetViewRange() const { return realViewRange_; }

const TDCoordinateSystem2D &TDGraphicEngineScreenState::GetWorldSpaceRange() const { return realWorldSpaceRange_; }

const TDCoordinateSystem2D &TDGraphicEngineScreenState::GetWorkSpaceRange() const { return realWorkSpaceRange_; }
