...
VEDBinaryReader::VEDBinaryReader(std::span<const std::byte> data) : data_(data) {}

VEDBinaryReader::VEDBinaryReader(const void *data, std::size_t size)
    : data_(data ? std::span<const std::byte>(static_cast<const std::byte *>(data), size)
                 : std::span<const std::byte>()),
      error_(data || size == 0 ? VEDBinaryError::None : VEDBinaryError::InvalidArgument) {}

bool VEDBinaryReader::Ok() const noexcept { return error_ == VEDBinaryError::None; }

VEDBinaryError VEDBinaryReader::Error() const noexcept { return error_; }

std::size_t VEDBinaryReader::Position() const noexcept { return position_; }

std::size_t VEDBinaryReader::Size() const noexcept { return data_.size(); }

std::size_t VEDBinaryReader::Remaining() const noexcept { return data_.size() - position_; }

bool VEDBinaryReader::End() const noexcept { return position_ == data_.size(); }

void VEDBinaryReader::ClearError() noexcept { error_ = VEDBinaryError::None; }
...
std::int8_t VEDBinaryReader::ReadInt8() { return static_cast<std::int8_t>(ReadUInt8()); }

std::uint8_t VEDBinaryReader::ReadUInt8() { return static_cast<std::uint8_t>(ReadLittleEndian(sizeof(std::uint8_t))); }

std::int16_t VEDBinaryReader::ReadInt16() { return static_cast<std::int16_t>(ReadUInt16()); }

std::uint16_t VEDBinaryReader::ReadUInt16() {
  return static_cast<std::uint16_t>(ReadLittleEndian(sizeof(std::uint16_t)));
}

std::int32_t VEDBinaryReader::ReadInt32() { return static_cast<std::int32_t>(ReadUInt32()); }
...
bool VEDBinaryReader::ReadBool() { return ReadUInt8() != 0; }

int VEDBinaryReader::ReadEnum() { return static_cast<int>(ReadInt32()); }
...
std::uint32_t VEDBinaryReader::ReadFourCC() { return ReadUInt32(); }
...
std::uint64_t VEDBinaryReader::ReadLittleEndian(std::size_t byteCount) {
  ...
  for (std::size_t i = 0; i < byteCount; ++i) {
    value |= static_cast<std::uint64_t>(std::to_integer<unsigned char>(data_[position_ + i]))
