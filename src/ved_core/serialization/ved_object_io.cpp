...
bool isSupportedCoreObjectType(std::uint32_t typeFourCC) {
  return typeFourCC == TDVecPolyCurve::StreamFourCC() || typeFourCC == TDVecLine::StreamFourCC() ||
         typeFourCC == TDVecEllipse::StreamFourCC() || typeFourCC == TDVecPolygon::StreamFourCC() ||
         typeFourCC == TDVecPolygonArea::StreamFourCC() || typeFourCC == TDVecBezierCurve::StreamFourCC() ||
         typeFourCC == TDVecBSPLine::StreamFourCC() || typeFourCC == TDVecGroup::StreamFourCC() ||
         typeFourCC == TDVecPolyCurveArea::StreamFourCC() || typeFourCC == TDVecText::StreamFourCC() ||
...
bool VEDObjectWriteResult::Ok() const noexcept { return error == VEDBinaryError::None; }

bool VEDObjectReadResult::Ok() const noexcept { return error == VEDBinaryError::None && object != nullptr; }
