...
void resolveTextFont(TDVecText &text, TDFontManager &fontManager, TDDocumentID docId, VEDFontResolutionResult &result) {
  ...
  if (TDVecFont *arialUnicode = fontManager.GetVecFontExact(kVEDArialUnicodeMSFontId, docId)) {
    ...
    result.warnings.push_back({VEDFontFallbackKind::RequestedMissingUsedArialUnicodeMS, requestedFontId,
  ...
  if (TDVecFont *segoeUI = fontManager.GetVecFontExact(kVEDSegoeUIFontId, docId)) {
    ...
    result.warnings.push_back({VEDFontFallbackKind::RequestedMissingUsedSegoeUI, requestedFontId,
  ...
  if (TDVecFont *wpsDefault = fontManager.GetVecFontExact(kVEDWpsDefaultFontId, nullptr)) {
    ...
    result.warnings.push_back({VEDFontFallbackKind::RequestedMissingUsedWpsDefault, requestedFontId,
                               wpsDefault->GetFontName() ? wpsDefault->GetFontName() : kVEDWpsDefaultFontId});
    return;
  }

  result.unresolvedFont = true;
  result.warnings.push_back({VEDFontFallbackKind::NoFallbackAvailable, requestedFontId, {}});
}

void resolveObjectFonts(TDVecObject &object, TDFontManager &fontManager, TDDocumentID docId,
  ...
  if (auto *group = dynamic_cast<TDVecGroup *>(&object)) {
    group->ForEachStoredObject(
        [&fontManager, docId, &result](TDVecObject &child) { resolveObjectFonts(child, fontManager, docId, result); });
  }
}

} // namespace

bool VEDFontResolutionResult::Ok() const noexcept { return !unresolvedFont; }
