...
TDVecBSPLine::TDVecBSPLine()
    : degree_(2), resolution_(50), showControls_(false), showPolygon_(false), leftControl_(0), topControl_(0),
      rightControl_(0), bottomControl_(0), height_(0.0), width_(0.0), numKnot_(0), upperMax_(0.0) {
  SetType(VECOBJ_BSPLINE);
}

std::uint32_t TDVecBSPLine::StreamFourCC() { return VEDMakeFourCC('b', 's', 'p', 'l'); }

std::uint32_t TDVecBSPLine::TypeFourCC() const { return StreamFourCC(); }
...
TDMatRect TDVecBSPLine::GetFrame() const { return frameFromPoints(controls_); }
...
std::unique_ptr<TDVecObject> TDVecBSPLine::Clone() const { return std::make_unique<TDVecBSPLine>(*this); }
...
long TDVecBSPLine::PointOnNode(TDMatPoint point, double tolerance) const { return PointOnFrameNode(point, tolerance); }
...
void TDVecBSPLine::MarkCurveDirty() { curveDirty_ = true; }
...
long TDVecBSPLine::PointOnControle(TDGraphicEngine *pGE, double X, double Y, long Toleranz) const {
  ...
  for (std::size_t i = 0; i < controls_.size(); ++i) {
    ...
    const double distanceToControl = std::hypot(static_cast<double>(pGE->RealToXPos(point.x) - xMouse),
...
long TDVecBSPLine::PointAfterControle(TDGraphicEngine *pGE, double X, double Y, long Toleranz) const {
  ...
  const TDMatPoint mouseScreen{static_cast<double>(pGE->RealToXPos(X)), static_cast<double>(pGE->RealToYPos(Y))};
  for (std::size_t i = 1; i < controls_.size(); ++i) {
    const TDMatPoint start{static_cast<double>(pGE->RealToXPos(controls_[i - 1].x)),
                           static_cast<double>(pGE->RealToYPos(controls_[i - 1].y))};
    const TDMatPoint end{static_cast<double>(pGE->RealToXPos(controls_[i].x)),
...
void TDVecBSPLine::SetShowControls(bool bShowControls) { showControls_ = bShowControls; }

bool TDVecBSPLine::GetShowControls() const { return showControls_; }

void TDVecBSPLine::SetShowPolygon(bool bShowPolygon) { showPolygon_ = bShowPolygon; }

bool TDVecBSPLine::GetShowPolygon() const { return showPolygon_; }
...
unsigned int TDVecBSPLine::GetDegree() const { return degree_; }
...
unsigned int TDVecBSPLine::GetResolution() const { return resolution_; }
...
int TDVecBSPLine::CountPoints() const { return static_cast<int>(controls_.size()); }
