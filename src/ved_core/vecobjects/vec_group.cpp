...
TDVecGroup::TDVecGroup()
    : rotateAngle_(0.0), xScale_(1.0), yScale_(1.0), topLeftBoundary_{0.0, 0.0}, bottomRightBoundary_{0.0, 0.0},
      originPoint_{0.0, 0.0}, xLength_(0.0), yLength_(0.0), objectInGroupCount_(0), resolveLock_(false), oldFrame_{} {
  SetType(VECOBJ_GRP);
}

std::uint32_t TDVecGroup::StreamFourCC() { return VEDMakeFourCC('g', 'r', 'u', 'p'); }

std::uint32_t TDVecGroup::TypeFourCC() const { return StreamFourCC(); }
...
TDVecGroup::TDVecGroup(const TDVecGroup &group)
    : TDVecObject(group), rotateAngle_(group.rotateAngle_), xScale_(group.xScale_), yScale_(group.yScale_),
      topLeftBoundary_(group.topLeftBoundary_), bottomRightBoundary_(group.bottomRightBoundary_),
      originPoint_(group.originPoint_), xLength_(group.xLength_), yLength_(group.yLength_),
      objectInGroupCount_(group.objectInGroupCount_), resolveLock_(group.resolveLock_), oldFrame_(group.oldFrame_) {
...
TDMatRect TDVecGroup::GetFrame() const {
  const TDMatRect diagonalFrame{
      {-xLength_, -yLength_}, {xLength_, -yLength_}, {xLength_, yLength_}, {-xLength_, yLength_}};
  const double angle = rotateAngle_;
  return {{(cosD(angle) * diagonalFrame.P1.x - sinD(angle) * diagonalFrame.P1.y) + originPoint_.x,
...
TDMatPoint TDVecGroup::GetMidpoint() const { return originPoint_; }

std::unique_ptr<TDVecObject> TDVecGroup::Clone() const { return std::make_unique<TDVecGroup>(*this); }
...
long TDVecGroup::PointOnNode(TDMatPoint point, double tolerance) const { return PointOnFrameNode(point, tolerance); }
...
long TDVecGroup::CountObjectsInGroup() const { return static_cast<long>(objectsInGroup_.size()); }
...
void TDVecGroup::SetResolveLock(bool bResolveLock) { resolveLock_ = bResolveLock; }

bool TDVecGroup::GetResolveLock() const { return resolveLock_; }
