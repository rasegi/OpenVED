...
long pointAfterConturScreenSegment(TDGraphicEngine *pGE, TDMatPoint point, const std::vector<TDMatConturPoint> &points,
                                   long tolerance) {
  if (!pGE || points.size() < 2) {
    return -1;
  }

  const TDMatPoint mouse{static_cast<double>(pGE->RealToXPos(point.x)), static_cast<double>(pGE->RealToYPos(point.y))};
  for (std::size_t i = 1; i < points.size(); ++i) {
    const TDMatPoint start{static_cast<double>(pGE->RealToXPos(points[i - 1].x)),
                           static_cast<double>(pGE->RealToYPos(points[i - 1].y))};
    const TDMatPoint end{static_cast<double>(pGE->RealToXPos(points[i].x)),
...
TDVecPolyCurveArea::TDVecPolyCurveArea()
    : rectangularLock_(false), resolution_(5), showControls_(false), showPolygon_(false) {
  SetType(VECOBJ_POLYCURVEAREA);
}

std::uint32_t TDVecPolyCurveArea::StreamFourCC() { return VEDMakeFourCC('p', 'c', 'a', 'r'); }

std::uint32_t TDVecPolyCurveArea::TypeFourCC() const { return StreamFourCC(); }
...
TDMatRect TDVecPolyCurveArea::GetFrame() const { return frameFromConturPoints(conturPoints_); }
...
std::unique_ptr<TDVecObject> TDVecPolyCurveArea::Clone() const { return std::make_unique<TDVecPolyCurveArea>(*this); }
...
long TDVecPolyCurveArea::PointOnNode(TDGraphicEngine *pGE, double X, double Y, long Toleranz) const {
  ...
  for (std::size_t i = 0; i < conturPoints_.size(); ++i) {
    ...
    const double nodeDistance = std::hypot(static_cast<double>(pGE->RealToXPos(point.x)) - mouseX,
...
void TDVecPolyCurveArea::SetRectangularLock(bool bValue) { rectangularLock_ = bValue; }

bool TDVecPolyCurveArea::GetRectangularLock() const { return rectangularLock_; }

void TDVecPolyCurveArea::SetShowControls(bool bShowControls) { showControls_ = bShowControls; }

bool TDVecPolyCurveArea::GetShowControls() const { return showControls_; }

void TDVecPolyCurveArea::SetShowPolygon(bool bShowPolygon) { showPolygon_ = bShowPolygon; }

bool TDVecPolyCurveArea::GetShowPolygon() const { return showPolygon_; }

void TDVecPolyCurveArea::SetResolution(unsigned int nResolution) { resolution_ = nResolution != 0 ? nResolution : 5; }

unsigned int TDVecPolyCurveArea::GetResolution() const { return resolution_; }
...
int TDVecPolyCurveArea::CountPoints() const { return static_cast<int>(conturPoints_.size()); }
