...
long pointAfterClosedScreenSegment(TDGraphicEngine *pGE, TDMatPoint point, const std::vector<TDMatPoint> &points,
                                   long tolerance) {
  ...
  for (std::size_t i = 1; i < closedPoints.size(); ++i) {
    const TDMatPoint start{static_cast<double>(pGE->RealToXPos(closedPoints[i - 1].x)),
                           static_cast<double>(pGE->RealToYPos(closedPoints[i - 1].y))};
    const TDMatPoint end{static_cast<double>(pGE->RealToXPos(closedPoints[i].x)),
...
TDVecPolygonArea::TDVecPolygonArea() : rectangularLock_(false) { SetType(VECOBJ_POLAREA); }

std::uint32_t TDVecPolygonArea::StreamFourCC() { return VEDMakeFourCC('p', 'o', 'l', 'a'); }

std::uint32_t TDVecPolygonArea::TypeFourCC() const { return StreamFourCC(); }
...
TDMatRect TDVecPolygonArea::GetFrame() const { return frameFromPoints(points_); }
...
std::unique_ptr<TDVecObject> TDVecPolygonArea::Clone() const { return std::make_unique<TDVecPolygonArea>(*this); }
...
TDVecLineHitResult TDVecPolygonArea::HitTestNode(TDGraphicEngine *pGE, TDMatPoint point, long tolerancePixels) const {
  ...
  double bestDistance = std::numeric_limits<double>::infinity();
  for (const TDMatPoint &candidate : points_) {
    bestDistance = std::min(bestDistance, std::hypot(static_cast<double>(pGE->RealToXPos(candidate.x)) - mouseX,
...
void TDVecPolygonArea::SetRectangularLock(bool bValue) { rectangularLock_ = bValue; }

bool TDVecPolygonArea::GetRectangularLock() const { return rectangularLock_; }
...
int TDVecPolygonArea::CountPoints() const { return static_cast<int>(points_.size()); }
