...
TDVecPolyCurve::TDVecPolyCurve() : resolution_(5), showControls_(true), showPolygon_(true) {
...
void TDVecPolyCurve::MarkDrawCacheDirty() { drawCacheDirty_ = true; }
...
std::uint32_t TDVecPolyCurve::StreamFourCC() { return VEDMakeFourCC('p', 'l', 'y', 'c'); }

std::uint32_t TDVecPolyCurve::TypeFourCC() const { return StreamFourCC(); }
...
TDMatRect TDVecPolyCurve::GetFrame() const { return frameFromConturPoints(conturPoints_); }
...
std::unique_ptr<TDVecObject> TDVecPolyCurve::Clone() const { return std::make_unique<TDVecPolyCurve>(*this); }
...
void TDVecPolyCurve::SetShowControls(bool bShowControls) { showControls_ = bShowControls; }

bool TDVecPolyCurve::GetShowControls() const { return showControls_; }

void TDVecPolyCurve::SetShowPolygon(bool bShowPolygon) { showPolygon_ = bShowPolygon; }

bool TDVecPolyCurve::GetShowPolygon() const { return showPolygon_; }
...
unsigned int TDVecPolyCurve::GetResolution() const { return resolution_; }
...
bool TDVecPolyCurve::RemovePoint(int iPoint) {
  ...
  if (current.eType == CPT_PRIME_LINE) {
    if (iPoint > 0 && iPoint < CountPoints() - 1) {
      ...
      return false;
    } else if (iPoint == CountPoints() - 1 && CountPoints() > 1 &&
...
int TDVecPolyCurve::CountPoints() const { return static_cast<int>(conturPoints_.size()); }
