...
double atanD2(double y, double x) { return std::atan2(y, x) * RAD_TO_DEG; }

TDMatRect makeFrameFromBounds(double left, double top, double right, double bottom) {
  return {{left, top}, {right, top}, {right, bottom}, {left, bottom}};
}

double distance(TDMatPoint a, TDMatPoint b) { return std::hypot(a.x - b.x, a.y - b.y); }
...
TDMatRect frameMidEdges(TDMatRect frame) {
  return {{(frame.P1.x + frame.P2.x) / 2.0, (frame.P1.y + frame.P2.y) / 2.0},
...
TDMatPoint frameNode(TDMatRect frame, long node) { return frameNode(frame, frameMidEdges(frame), node); }

bool mouseOnNode(POINT node, long nodeHeight, POINT mouse, long tolerance) {
  return mouse.x >= node.x - nodeHeight - tolerance && mouse.x <= node.x + nodeHeight + tolerance &&
         mouse.y >= node.y - nodeHeight - tolerance && mouse.y <= node.y + nodeHeight + tolerance;
}

bool realPointOnNode(TDMatPoint node, TDMatPoint point, double tolerance) {
  return point.x >= node.x - tolerance && point.x <= node.x + tolerance && point.y >= node.y - tolerance &&
...
double distancePointToSegment(double pointX, double pointY, double startX, double startY, double endX, double endY) {
  ...
  const double t = std::clamp(((pointX - startX) * dx + (pointY - startY) * dy) / lengthSquared, 0.0, 1.0);
...
TDMatPoint ellipsePoint(double xCenter, double yCenter, double xRadius, double yRadius, double angle, double radians) {
  ...
  return {cosD(angle) * localX + sinD(angle) * localY + xCenter,
