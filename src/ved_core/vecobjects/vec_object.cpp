...
long pointOnFrameNode(const TDMatRect &frame, TDGraphicEngine *pGE, double X, double Y, long Toleranz) {
  if (!pGE) {
    return -1;
  }
  ...
  const std::array<POINT, 8> nodes{{{pGE->RealToXPos(frame.P1.x), pGE->RealToYPos(frame.P1.y)},
...
std::array<TDMatPoint, 8> frameNodes(const TDMatRect &frame) {
  const TDMatRect middle = frameMidEdges(frame);
  return {{frame.P1, middle.P1, frame.P2, middle.P2, frame.P3, middle.P3, frame.P4, middle.P4}};
}
...
bool TDVecLineHitResult::IsHit() const { return kind != TDVecLineHitKind::None; }

bool TDVecLineHitResult::IsNodeHit() const {
  return kind == TDVecLineHitKind::StartNode || kind == TDVecLineHitKind::EndNode ||
         kind == TDVecLineHitKind::MidpointNode;
}

TDVecObject::TDVecObject()
    : type_(VECOBJ_UNKNOWN), selected_(false), lockResize_(false), resizeProportional_(false), lockPosition_(false),
      color_(0), parentModel_(nullptr) {}
...
TDMatRect TDVecObject::GetScaleFrame() const { return GetFrame(); }
...
TDVecObjectType TDVecObject::GetType() const { return type_; }
...
void TDVecObject::TransformToPoint(TDMatPoint MatPoint) { (void)MatPoint; }

void TDVecObject::TransformToOrigin(TDMatPoint MatPoint) { (void)MatPoint; }

std::uint32_t TDVecObject::TypeFourCC() const { return 0; }
...
void TDVecObject::SetSelect(bool bSelect) { selected_ = bSelect; }

bool TDVecObject::GetSelect() const { return selected_; }

void TDVecObject::SetColor(TDRgbColor color) { color_ = color; }

TDRgbColor TDVecObject::GetColor() const { return color_; }

void TDVecObject::Initialize() {}

void TDVecObject::SetLockResize(bool bLockResize) { lockResize_ = bLockResize; }

bool TDVecObject::GetLockResize() const { return lockResize_; }

void TDVecObject::SetResizeProportional(bool bValue) { resizeProportional_ = bValue; }

bool TDVecObject::GetResizeProportional() const { return resizeProportional_; }

void TDVecObject::SetLockPosition(bool bLockPosition) { lockPosition_ = bLockPosition; }

bool TDVecObject::GetLockPosition() const { return lockPosition_; }

bool TDVecObject::GetLockObject() const { return lockResize_ || lockPosition_; }

void TDVecObject::SetParentModel(TDVecModel *pParentModel) { parentModel_ = pParentModel; }

TDVecModel *TDVecObject::GetParentModel() const { return parentModel_; }

void TDVecObject::SetType(TDVecObjectType eType) { type_ = eType; }
