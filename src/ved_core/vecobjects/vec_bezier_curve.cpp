...
TDVecBezierCurve::TDVecBezierCurve()
    : resolution_(20), showControls_(false), showPolygon_(false), leftControl_(0), topControl_(0), rightControl_(0),
      bottomControl_(0), height_(0.0), width_(0.0) {
  SetType(VECOBJ_BZC);
}

std::uint32_t TDVecBezierCurve::StreamFourCC() { return VEDMakeFourCC('b', 'z', 'c', 'r'); }

std::uint32_t TDVecBezierCurve::TypeFourCC() const { return StreamFourCC(); }
...
TDMatRect TDVecBezierCurve::GetFrame() const { return frameFromPoints(controls_); }
...
std::unique_ptr<TDVecObject> TDVecBezierCurve::Clone() const { return std::make_unique<TDVecBezierCurve>(*this); }
...
void TDVecBezierCurve::SetShowControls(bool bShowControls) { showControls_ = bShowControls; }

bool TDVecBezierCurve::GetShowControls() const { return showControls_; }

void TDVecBezierCurve::SetShowPolygon(bool bShowPolygon) { showPolygon_ = bShowPolygon; }

bool TDVecBezierCurve::GetShowPolygon() const { return showPolygon_; }

void TDVecBezierCurve::SetResolution(unsigned int nResolution) { resolution_ = nResolution != 0 ? nResolution : 20; }

unsigned int TDVecBezierCurve::GetResolution() const { return resolution_; }
...
int TDVecBezierCurve::CountPoints() const { return static_cast<int>(controls_.size()); }
