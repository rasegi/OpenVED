...
TDVecLine::TDVecLine() : TDVecLine(0.0, 0.0, 0.0, 0.0) {}

TDVecLine::TDVecLine(double x1, double y1, double x2, double y2) : x1_(x1), y1_(y1), x2_(x2), y2_(y2) {
  SetType(VECOBJ_LIN);
}

std::uint32_t TDVecLine::StreamFourCC() { return VEDMakeFourCC('l', 'i', 'n', 'e'); }

std::uint32_t TDVecLine::TypeFourCC() const { return StreamFourCC(); }
...
TDMatRect TDVecLine::GetFrame() const {
  return makeFrameFromBounds(std::min(x1_, x2_), std::min(y1_, y2_), std::max(x1_, x2_), std::max(y1_, y2_));
}

TDMatPoint TDVecLine::GetMidpoint() const { return {(x1_ + x2_) / 2.0, (y1_ + y2_) / 2.0}; }

std::unique_ptr<TDVecObject> TDVecLine::Clone() const { return std::make_unique<TDVecLine>(*this); }
...
TDVecLineHitResult TDVecLine::HitTestNode(TDMatPoint point, double tolerance) const {
  ...
  const TDMatPoint end{x2_, y2_};
  const TDMatPoint midpoint = GetMidpoint();
  const std::array candidates{TDVecLineHitResult{TDVecLineHitKind::StartNode, distance(point, start)},
...
TDVecLineHitResult TDVecLine::HitTestNode(TDGraphicEngine *pGE, TDMatPoint point, long tolerancePixels) const {
  ...
  const std::array candidates{
      TDVecLineHitResult{TDVecLineHitKind::StartNode, std::hypot(static_cast<double>(pGE->RealToXPos(x1_)) - mouseX,
                                                                 static_cast<double>(pGE->RealToYPos(y1_)) - mouseY)},
      TDVecLineHitResult{TDVecLineHitKind::EndNode, std::hypot(static_cast<double>(pGE->RealToXPos(x2_)) - mouseX,
                                                               static_cast<double>(pGE->RealToYPos(y2_)) - mouseY)},
      TDVecLineHitResult{TDVecLineHitKind::MidpointNode,
                         std::hypot(static_cast<double>(pGE->RealToXPos(midpoint.x)) - mouseX,
                                    static_cast<double>(pGE->RealToYPos(midpoint.y)) - mouseY)}};
...
bool TDVecLine::MoveNode(TDVecLineHitKind nodeKind, TDMatPoint point) {
  if (GetLockResize() && (nodeKind == TDVecLineHitKind::StartNode || nodeKind == TDVecLineHitKind::EndNode)) {
    return false;
  }
...
TDMatLine TDVecLine::GetLine() const { return {x1_, y1_, x2_, y2_}; }
