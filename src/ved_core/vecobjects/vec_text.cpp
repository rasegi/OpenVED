...
void expandFrame(TDMatRect &frame, const TDMatRect &next, bool &initialized) {
  ...
  frame = rectFromBounds(std::min(currentLeft, left), std::min(currentTop, top), std::max(currentRight, right),
                         std::max(currentBottom, bottom));
}

TDMatPoint midpoint(const TDMatRect &rect) { return {(rect.P1.x + rect.P3.x) / 2.0, (rect.P1.y + rect.P3.y) / 2.0}; }
...
void transformRectToPoint(TDMatRect &rect, TDMatPoint point) { moveRect(rect, -point.x, -point.y); }

void transformRectToOrigin(TDMatRect &rect, TDMatPoint point) { moveRect(rect, point.x, point.y); }
...
unsigned int decodeNextUtf8OrLatin1(const char *&text) {
  ...
  if ((bytes[0] & 0xF8U) == 0xF0U && (bytes[1] & 0xC0U) == 0x80U && (bytes[2] & 0xC0U) == 0x80U &&
      (bytes[3] & 0xC0U) == 0x80U) {
...
TDVecText::TDVecText()
    : mOriginPoint{0.0, 0.0}, mnXScale(1.0), mnYScale(1.0), mnAngle(0.0), mnIncline(0.0), mnLineSpacing(1.0),
      mnCharSpacing(1.0), mbVertical(false), mbUnderline(false), meJustification(JUST_LEFT),
      meVerticalAlignment(VALIGN_TOP), mnResolution(MID_TEXT_RESOLUTION), msFontName{}, mpVecFont(gDefaultVecTextFont),
      msText(new char[1]) {
  msText[0] = '\0';
  SetType(VECOBJ_TEX);
  if (mpVecFont) {
...
TDVecText::TDVecText(const TDVecText &other) : TDVecObject(other), msText(nullptr) { CopyFrom(other); }
...
TDVecText::~TDVecText() { delete[] msText; }
...
std::uint32_t TDVecText::StreamFourCC() { return VEDMakeFourCC('v', 't', 'x', 't'); }

std::uint32_t TDVecText::TypeFourCC() const { return StreamFourCC(); }
...
TDMatPoint TDVecText::GetMidpoint() const { return midpoint(GetFrame()); }

std::unique_ptr<TDVecObject> TDVecText::Clone() const { return std::make_unique<TDVecText>(*this); }

TDVecLineHitResult TDVecText::HitTest(TDMatPoint point, double tolerance) const {
  return frameContainsPoint(GetFrame(), point, tolerance) ? TDVecLineHitResult{TDVecLineHitKind::Body, 0.0}
                                                          : TDVecLineHitResult{};
}
...
long TDVecText::PointOnNode(TDMatPoint point, double tolerance) const { return PointOnFrameNode(point, tolerance); }
...
void TDVecText::MarkGlyphsDirty() { glyphsDirty_ = true; }
...
void TDVecText::SetXScale(double nXScale) {
  if (!MatBelike2Double(nXScale, 0.0, 4)) {
    mnXScale = nXScale;
    MarkGlyphsDirty();
  }
}
double TDVecText::GetXScale() const { return mnXScale; }
void TDVecText::SetYScale(double nYScale) {
  if (!MatBelike2Double(nYScale, 0.0, 4)) {
    mnYScale = nYScale;
    MarkGlyphsDirty();
  }
}
double TDVecText::GetYScale() const { return mnYScale; }
void TDVecText::SetAngle(double nAngle) {
  mnAngle = nAngle;
  MarkGlyphsDirty();
}
double TDVecText::GetAngle() const { return mnAngle; }
void TDVecText::SetIncline(double nIncline) {
  mnIncline = nIncline;
  MarkGlyphsDirty();
}
double TDVecText::GetIncline() const { return mnIncline; }
void TDVecText::SetLineSpacing(double nLineSpacing) {
  mnLineSpacing = nLineSpacing;
  MarkGlyphsDirty();
}
double TDVecText::GetLineSpacing() const { return mnLineSpacing; }
void TDVecText::SetCharSpacing(double nCharSpacing) {
  mnCharSpacing = nCharSpacing;
  MarkGlyphsDirty();
}
...
void TDVecText::SetJustification(TDJustification eJustification) {
  meJustification = eJustification;
  MarkGlyphsDirty();
}
TDVecText::TDJustification TDVecText::GetJustification() const { return meJustification; }
void TDVecText::SetVerticalAlignment(TDVerticalAlignment eVerticalAlignment) {
...
void TDVecText::SetVecFontPointer(const TDVecFont *pVecFont) {
  mpVecFont = pVecFont;
  MarkGlyphsDirty();
}
const TDVecFont *TDVecText::GetVecFontPointer() const { return mpVecFont; }
void TDVecText::SetResolution(unsigned int nResolution) {
  mnResolution = nResolution <= MAX_TEXT_RESOLUTION ? nResolution : MID_TEXT_RESOLUTION;
}
...
int TDVecText::CountGlyphs() const {
  EnsureGlyphsInitialized();
...
TDVecFrameText::TDVecFrameText() : TDVecText(), mRectangle{}, mbOriginIsjustified(false), mbScaleDependency(true) {
  SetType(VECOBJ_FRAMETEXT);
}

TDVecFrameText::TDVecFrameText(const TDVecFrameText &other)
    : TDVecText(other), mRectangle(other.mRectangle), mbOriginIsjustified(other.mbOriginIsjustified),
...
std::uint32_t TDVecFrameText::StreamFourCC() { return VEDMakeFourCC('f', 't', 'x', 't'); }

std::uint32_t TDVecFrameText::TypeFourCC() const { return StreamFourCC(); }
...
TDMatPoint TDVecFrameText::GetMidpoint() const { return midpoint(mRectangle); }

std::unique_ptr<TDVecObject> TDVecFrameText::Clone() const { return std::make_unique<TDVecFrameText>(*this); }
...
void SetDefaultVecTextFont(const TDVecFont *font) { gDefaultVecTextFont = font; }

const TDVecFont *GetDefaultVecTextFont() { return gDefaultVecTextFont; }

void SetVecTextShaper(TDVecTextShaper shaper) { gVecTextShaper = std::move(shaper); }
