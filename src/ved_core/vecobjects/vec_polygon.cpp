...
long pointAfterScreenSegment(TDGraphicEngine *pGE, TDMatPoint point, const std::vector<TDMatPoint> &points,
                             long tolerance) {
...
TDVecPolygon::TDVecPolygon() { SetType(VECOBJ_POL); }

std::uint32_t TDVecPolygon::StreamFourCC() { return VEDMakeFourCC('p', 'o', 'l', 'y'); }

std::uint32_t TDVecPolygon::TypeFourCC() const { return StreamFourCC(); }
...
TDMatRect TDVecPolygon::GetFrame() const { return frameFromPoints(points_); }
...
std::unique_ptr<TDVecObject> TDVecPolygon::Clone() const { return std::make_unique<TDVecPolygon>(*this); }
...
long TDVecPolygon::PointOnNode(TDGraphicEngine *pGE, double X, double Y, long Toleranz) const {
  ...
  for (std::size_t i = 0; i < points_.size(); ++i) {
    const double nodeDistance = std::hypot(static_cast<double>(pGE->RealToXPos(points_[i].x)) - mouseX,
...
int TDVecPolygon::CountPoints() const { return static_cast<int>(points_.size()); }
