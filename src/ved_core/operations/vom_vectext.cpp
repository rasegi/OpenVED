...
namespace {
void copyFontName(char *destination, const char *source) { std::snprintf(destination, 30, "%s", source ? source : ""); }

bool isTextObjectType(TDVecObjectType type) { return type == VECOBJ_TEX || type == VECOBJ_FRAMETEXT; }
...
TDVOMVecText::TDVOMVecText(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                           TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), mOriginPoint{}, mpVOMVecTextExtVar(nullptr) {
...
TDVOMVecText::TDVOMVecText(const TDVOMVecText &oldOperation)
    : TDVOModify(oldOperation.mpVecModel, oldOperation.mpVecEditCad, oldOperation.mpParentOperationManager),
      mOriginPoint(oldOperation.mOriginPoint), mpVOMVecTextExtVar(nullptr) {
...
TDVOMVecText *TDVOMVecText::Clone() const { return new TDVOMVecText(*this); }
...
bool TDVOMVecText::TextIsOK() const { return mpVOMVecTextExtVar && mpVOMVecTextExtVar->TextIsOK(); }

TDVOMVecTextExtVar::TDVOMVecTextExtVar(TDViewOperation *pParentOperation)
    : TDVOPExternalVariables(pParentOperation), mParamValidity{}, mnXScale(1.0), mnYScale(1.0), mnAngle(0.0),
      mnIncline(0.0), mnLineSpacing(1.0), mnCharSpacing(1.0), mbVertical(false), mbUnderline(false),
      meJustification(TDVecText::JUST_LEFT), meVerticalAlignment(TDVecText::VALIGN_TOP), mnResolution(4), msFontName{},
      mpVecFont(nullptr), mbScaleDependency(true), msText(), mbHasText(false), mObjectType(VECOBJ_UNKNOWN) {}

TDVOMVecTextExtVar::TDVOMVecTextExtVar(const TDVOMVecTextExtVar &oldExtVar)
    : TDVOPExternalVariables(oldExtVar), mParamValidity(oldExtVar.mParamValidity), mnXScale(oldExtVar.mnXScale),
      mnYScale(oldExtVar.mnYScale), mnAngle(oldExtVar.mnAngle), mnIncline(oldExtVar.mnIncline),
      mnLineSpacing(oldExtVar.mnLineSpacing), mnCharSpacing(oldExtVar.mnCharSpacing), mbVertical(oldExtVar.mbVertical),
      mbUnderline(oldExtVar.mbUnderline), meJustification(oldExtVar.meJustification),
      meVerticalAlignment(oldExtVar.meVerticalAlignment), mnResolution(oldExtVar.mnResolution), msFontName{},
      mpVecFont(oldExtVar.mpVecFont), mbScaleDependency(oldExtVar.mbScaleDependency), msText(oldExtVar.msText),
      mbHasText(oldExtVar.mbHasText), mObjectType(oldExtVar.mObjectType) {
...
TDVOMVecTextExtVar *TDVOMVecTextExtVar::Clone() const { return new TDVOMVecTextExtVar(*this); }
...
const char *TDVOMVecTextExtVar::GetText() const { return mbHasText ? msText.c_str() : nullptr; }
...
bool TDVOMVecTextExtVar::TextIsOK() const { return mbHasText; }

TDVecObjectType TDVOMVecTextExtVar::GetObjectType() const { return mObjectType; }
