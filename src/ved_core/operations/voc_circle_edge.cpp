...
TDVOCCircleEdge::TDVOCCircleEdge(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
...
bool TDVOCCircleEdge::UpdatePreviewCircle() {
  mbCircleOK =
      MatCircleKanteReal(mPtBeg, mPtPoint, mPtEnd, &mMatCircle) && mMatCircle.r > minimumDistance(mpVecEditCad);
  if (mbCircleOK) {
...
bool TDVOCCircleEdge::UpdateFinalCircle(TDMatPoint edgePoint) {
  mPtEnd = edgePoint;
  mbCircleOK =
      MatCircleKanteReal(mPtBeg, mPtEnd, mPtPoint, &mMatCircle) && mMatCircle.r > minimumDistance(mpVecEditCad);
  return mbCircleOK;
}
