...
TDVOCVecText::TDVOCVecText(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                           TDViewOperationManager *pParentOperationManager)
    : TDVOCreate(pVecModel, pVecEditCad, pParentOperationManager), mOriginPoint{},
      mpObjText(std::make_unique<TDVecText>()), mpVOCVecTextExtVar(nullptr) {
...
TDVOCVecText::TDVOCVecText(const TDVOCVecText &oldOperation)
    : TDVOCreate(oldOperation.mpVecModel, oldOperation.mpVecEditCad, oldOperation.mpParentOperationManager),
      ...
      mpObjText(oldOperation.mpObjText ? std::make_unique<TDVecText>(*oldOperation.mpObjText)
                                       : std::make_unique<TDVecText>()),
...
TDVOCVecText &TDVOCVecText::operator=(const TDVOCVecText &oldOperation) {
  ...
  mOriginPoint = oldOperation.mOriginPoint;
  mpObjText =
      oldOperation.mpObjText ? std::make_unique<TDVecText>(*oldOperation.mpObjText) : std::make_unique<TDVecText>();
...
TDVOCVecText *TDVOCVecText::Clone() const { return new TDVOCVecText(*this); }
...
bool TDVOCVecText::TextIsOK() const { return mpVOCVecTextExtVar && mpVOCVecTextExtVar->TextIsOK(); }
...
TDVOCVecTextExtVar::TDVOCVecTextExtVar(TDViewOperation *pParentOperation)
    : TDVOPExternalVariables(pParentOperation), mbTextIsDeleted(false), mnXScale(1.0), mnYScale(1.0), mnAngle(0.0),
      mnIncline(0.0), mnLineSpacing(1.0), mnCharSpacing(1.0), mbVertical(false), mbUnderline(false),
      meJustification(TDVecText::JUST_LEFT), meVerticalAlignment(TDVecText::VALIGN_TOP), msFontName{},
      mpVecFont(GetDefaultVecTextFont()), msText(), mnResolution(4) {
  if (mpVecFont) {
...
TDVOCVecTextExtVar::TDVOCVecTextExtVar(const TDVOCVecTextExtVar &oldExtVar)
    : TDVOPExternalVariables(oldExtVar), mbTextIsDeleted(oldExtVar.mbTextIsDeleted), mnXScale(oldExtVar.mnXScale),
      ...
      meVerticalAlignment(oldExtVar.meVerticalAlignment), msFontName{}, mpVecFont(oldExtVar.mpVecFont),
      msText(oldExtVar.msText), mnResolution(oldExtVar.mnResolution) {
...
TDVOCVecTextExtVar *TDVOCVecTextExtVar::Clone() const { return new TDVOCVecTextExtVar(*this); }
...
const char *TDVOCVecTextExtVar::GetText() const { return msText.empty() ? nullptr : msText.c_str(); }
...
bool TDVOCVecTextExtVar::TextIsOK() const { return !msText.empty() && !IsStringBlank(msText.c_str()); }

bool TDVOCVecTextExtVar::TextIsDeleted() const { return mbTextIsDeleted; }
