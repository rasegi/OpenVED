...
TDVOMInsertObjects::TDVOMInsertObjects(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                       TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), mpVOMInsertObjectsExtVar(nullptr),
      mpTmpVecGroup(nullptr), mpTmpObject(nullptr), mbGroupMove(false), mbDragOK(false) {
...
TDVOMInsertObjects::TDVOMInsertObjects(const TDVOMInsertObjects &oldOperation)
    : TDVOModify(oldOperation), mpVOMInsertObjectsExtVar(nullptr),
      mpTmpVecGroup(oldOperation.mpTmpVecGroup ? std::unique_ptr<TDVecGroup>(dynamic_cast<TDVecGroup *>(
                                                     oldOperation.mpTmpVecGroup->Clone().release()))
                                               : nullptr),
      mpTmpObject(oldOperation.mpTmpObject ? oldOperation.mpTmpObject->Clone() : nullptr),
      mbGroupMove(oldOperation.mbGroupMove), mbDragOK(oldOperation.mbDragOK) {
  mpVOPExternalVariables = oldOperation.mpVOMInsertObjectsExtVar ? oldOperation.mpVOMInsertObjectsExtVar->Clone()
...
TDVOMInsertObjects *TDVOMInsertObjects::Clone() const { return new TDVOMInsertObjects(*this); }

void TDVOMInsertObjects::Initialize() { SetState(OSTATE_STARTED); }
...
void __fastcall TDVOMInsertObjects::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                double Y) {
  (void)Shift;
  (void)X;
  (void)Y;
...
void __fastcall TDVOMInsertObjects::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                double Y) {
  (void)Button;
  (void)Shift;
  if (!mbDragOK) {
...
TDVOMInsertObjectsExtVar::TDVOMInsertObjectsExtVar(TDViewOperation *pParentOperation)
    : TDVOPExternalVariables(pParentOperation), objectsForInserting_() {}

TDVOMInsertObjectsExtVar::TDVOMInsertObjectsExtVar(const TDVOMInsertObjectsExtVar &oldExtVar)
    : TDVOPExternalVariables(oldExtVar), objectsForInserting_() {
...
TDVOMInsertObjectsExtVar *TDVOMInsertObjectsExtVar::Clone() const { return new TDVOMInsertObjectsExtVar(*this); }
