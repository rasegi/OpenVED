...
TDVOCBezierCurveControlPoint::TDVOCBezierCurveControlPoint(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                                           TDViewOperationManager *pParentOperationManager)
    : TDVOCreate(pVecModel, pVecEditCad, pParentOperationManager),
      mpObjBezierCurve(std::make_unique<TDVecBezierCurve>()), mpTmpLine(std::make_unique<TDVecLine>()),
      mpTmpPolygon(nullptr), MatLine{}, mMatPoints() {
...
TDVOCBezierCurveControlPoint::TDVOCBezierCurveControlPoint(const TDVOCBezierCurveControlPoint &oldOperation)
    : TDVOCreate(oldOperation),
      mpObjBezierCurve(
          oldOperation.mpObjBezierCurve ? std::make_unique<TDVecBezierCurve>(*oldOperation.mpObjBezierCurve) : nullptr),
      ...
      MatLine(oldOperation.MatLine), mMatPoints(oldOperation.mMatPoints) {}

TDVOCBezierCurveControlPoint &
TDVOCBezierCurveControlPoint::operator=(const TDVOCBezierCurveControlPoint &oldOperation) {
...
void __fastcall TDVOCBezierCurveControlPoint::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                          double Y) {
  switch (GetState()) {
  case OSTATE_STARTED:
    if (mMatPoints.empty()) {
...
void __fastcall TDVOCBezierCurveControlPoint::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                        double Y) {
  (void)X;
  (void)Y;
...
void __fastcall TDVOCBezierCurveControlPoint::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                          double Y) {
  (void)Shift;
