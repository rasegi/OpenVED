...
TDVOCEllipseMidpoint::TDVOCEllipseMidpoint(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                           TDViewOperationManager *pParentOperationManager)
    : TDVOCreate(pVecModel, pVecEditCad, pParentOperationManager), mpObjEllipse(std::make_unique<TDVecEllipse>()),
      mpTmpLine(std::make_unique<TDVecLine>()), mbAxeOK(false), mbEllipseOK(false) {
...
bool TDVOCEllipseMidpoint::IsAxisValid() const { return mMatLine.Length() > minimumDistance(mpVecEditCad); }
...
void __fastcall TDVOCEllipseMidpoint::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                  double Y) {
  (void)Shift;
  switch (GetState()) {
  case OSTATE_STARTED:
...
void __fastcall TDVOCEllipseMidpoint::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                double Y) {
  (void)Button;
  (void)Shift;
  switch (mClick) {
...
void __fastcall TDVOCEllipseMidpoint::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                  double Y) {
  (void)Shift;
