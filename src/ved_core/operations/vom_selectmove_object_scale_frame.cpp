...
TDVOMSelectMoveObjectScaleFrame::TDVOMSelectMoveObjectScaleFrame(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                                                 TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), mpVOMSelectMoveObjScaleFrameExtVar(nullptr),
      mbMoveObject(false), mbMoveNode(false), mbObjectChanged(false), mbMustViewsRefresh(false), mbDrawLassoOK(false),
      mbGroupMove(false), mnNodeMove(-1), miObjIndex(-1), mbToleranzOK(false), mbDialogOpen(true) {
...
TDVOMSelectMoveObjectScaleFrame::TDVOMSelectMoveObjectScaleFrame(const TDVOMSelectMoveObjectScaleFrame &oldOperation)
    : TDVOModify(oldOperation),
      mpVOMSelectMoveObjScaleFrameExtVar(dynamic_cast<TDVOMSelectMoveObjScaleFrameExtVar *>(mpVOPExternalVariables)),
      mbMoveObject(oldOperation.mbMoveObject), mbMoveNode(oldOperation.mbMoveNode),
      mbObjectChanged(oldOperation.mbObjectChanged), mbMustViewsRefresh(oldOperation.mbMustViewsRefresh),
      mbDrawLassoOK(oldOperation.mbDrawLassoOK), mbGroupMove(oldOperation.mbGroupMove),
      mnNodeMove(oldOperation.mnNodeMove), miObjIndex(oldOperation.miObjIndex), mbToleranzOK(oldOperation.mbToleranzOK),
...
TDVOMSelectMoveObjectScaleFrame &
TDVOMSelectMoveObjectScaleFrame::operator=(const TDVOMSelectMoveObjectScaleFrame &oldOperation) {
...
long TDVOMSelectMoveObjectScaleFrame::FindObjectFrame(TDMatPoint point) const {
  ...
  for (int index = 0; index < mpVecModel->CountObjects(); ++index) {
    ...
    if (graphicEngine) {
      ...
      } else if (object->GetSelect() && object->PointOnScaleFrameNode(graphicEngine, point.x, point.y,
                                                                      graphicEngine->GetMouseTolerance()) > -1) {
        selectedElement = index;
      }
    } else {
...
void __fastcall TDVOMSelectMoveObjectScaleFrame::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift,
                                                             double X, double Y) {
  ...
  if (!mbGroupMove && selectedObject && mpVecModel->GetSelectedObjectIndex() != -1 &&
      (Shift & VKState_KEY_SHIFT) == 0) {
...
void __fastcall TDVOMSelectMoveObjectScaleFrame::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                           double Y) {
  ...
  if (!mbMoveNode && !mbMoveObject && !mbObjectChanged) {
    if (mbDrawLassoOK) {
      mpVecModel->SelectObjectsInArea({XMouseStart, YMouseStart}, {XMouseJet, YMouseJet},
                                      (Shift & VKState_KEY_SHIFT) != 0);
    }
    FinishInteraction();
    return;
...
void __fastcall TDVOMSelectMoveObjectScaleFrame::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift,
                                                             double X, double Y) {
  (void)Shift;
  if (Button != VIRTMOUSEBTM_LEFT) {
    return;
...
TDVOMSelectMoveObjScaleFrameExtVar::TDVOMSelectMoveObjScaleFrameExtVar(TDViewOperation *pParentOperation)
    : TDVOPExternalVariables(pParentOperation), mFrame{}, mnHeight(0.0), mnWidth(0.0), mnObjectsCount(0) {}

TDVOMSelectMoveObjScaleFrameExtVar::TDVOMSelectMoveObjScaleFrameExtVar(
    const TDVOMSelectMoveObjScaleFrameExtVar &oldExtVar)
    : TDVOPExternalVariables(oldExtVar), mFrame(oldExtVar.mFrame), mnHeight(oldExtVar.mnHeight),
      mnWidth(oldExtVar.mnWidth), mnObjectsCount(oldExtVar.mnObjectsCount) {}

TDVOMSelectMoveObjScaleFrameExtVar &
TDVOMSelectMoveObjScaleFrameExtVar::operator=(const TDVOMSelectMoveObjScaleFrameExtVar &oldExtVar) {
  if (this == &oldExtVar) {
    return *this;
  }
...
void TDVOMSelectMoveObjScaleFrameExtVar::SetFrame(TDMatRect Frame) { mFrame = Frame; }

TDMatRect TDVOMSelectMoveObjScaleFrameExtVar::GetFrame() const { return mFrame; }

void TDVOMSelectMoveObjScaleFrameExtVar::SetHeight(double nHeight) { mnHeight = nHeight; }

double TDVOMSelectMoveObjScaleFrameExtVar::GetHeight() const { return mnHeight; }

void TDVOMSelectMoveObjScaleFrameExtVar::SetWidth(double nWidth) { mnWidth = nWidth; }

double TDVOMSelectMoveObjScaleFrameExtVar::GetWidth() const { return mnWidth; }

void TDVOMSelectMoveObjScaleFrameExtVar::SetObjectsCount(long nObjectsCount) { mnObjectsCount = nObjectsCount; }

long TDVOMSelectMoveObjScaleFrameExtVar::GetObjectsCount() const { return mnObjectsCount; }
