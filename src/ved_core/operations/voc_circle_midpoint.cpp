...
TDVOCCircleMidpoint::TDVOCCircleMidpoint(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                         TDViewOperationManager *pParentOperationManager)
    : TDVOCreate(pVecModel, pVecEditCad, pParentOperationManager), mpObjCircle(std::make_unique<TDVecEllipse>()),
      mbCircleOK(false) {
...
bool TDVOCCircleMidpoint::IsCircleValid() const { return mMatCircle.r > minimumDistance(mpVecEditCad); }
...
void __fastcall TDVOCCircleMidpoint::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                 double Y) {
  (void)Shift;
  switch (GetState()) {
  case OSTATE_STARTED:
...
void __fastcall TDVOCCircleMidpoint::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                 double Y) {
  (void)Shift;
