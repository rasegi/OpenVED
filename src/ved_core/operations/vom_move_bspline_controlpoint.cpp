...
TDVOMMoveBSPLineControlPoint::TDVOMMoveBSPLineControlPoint(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                                           TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), mpTmpObject(nullptr), mbDragOK(false),
      miControle(-1) {
...
TDVOMMoveBSPLineControlPoint::TDVOMMoveBSPLineControlPoint(const TDVOMMoveBSPLineControlPoint &oldOperation)
    : TDVOModify(oldOperation), mpTmpObject(oldOperation.mpTmpObject ? oldOperation.mpTmpObject->Clone() : nullptr),
      mbDragOK(oldOperation.mbDragOK), miControle(oldOperation.miControle) {}

TDVOMMoveBSPLineControlPoint &
TDVOMMoveBSPLineControlPoint::operator=(const TDVOMMoveBSPLineControlPoint &oldOperation) {
...
void __fastcall TDVOMMoveBSPLineControlPoint::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                          double Y) {
  (void)Shift;
...
void __fastcall TDVOMMoveBSPLineControlPoint::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                        double Y) {
  (void)Shift;
  if (Button == VIRTMOUSEBTM_RIGHT) {
    mbDragOK = false;
...
void __fastcall TDVOMMoveBSPLineControlPoint::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                          double Y) {
  (void)Shift;
