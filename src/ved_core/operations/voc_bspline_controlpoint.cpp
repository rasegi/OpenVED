...
TDVOCBSPLineControlPoint::TDVOCBSPLineControlPoint(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                                   TDViewOperationManager *pParentOperationManager)
    : TDVOCreate(pVecModel, pVecEditCad, pParentOperationManager), mpVOCBSPLineControlPtExtVar(nullptr),
      mpObjBSPLine(std::make_unique<TDVecBSPLine>()), mpTmpObjBSPLine(nullptr), mMatPoints(), mnDegree(2),
      mbShowPolygon(true), mbShowControls(true), mnResolution(50) {
...
TDVOCBSPLineControlPoint::TDVOCBSPLineControlPoint(const TDVOCBSPLineControlPoint &oldOperation)
    : TDVOCreate(oldOperation), mpVOCBSPLineControlPtExtVar(nullptr),
      mpObjBSPLine(oldOperation.mpObjBSPLine ? std::make_unique<TDVecBSPLine>(*oldOperation.mpObjBSPLine) : nullptr),
      mpTmpObjBSPLine(oldOperation.mpTmpObjBSPLine ? std::make_unique<TDVecBSPLine>(*oldOperation.mpTmpObjBSPLine)
                                                   : nullptr),
      mMatPoints(oldOperation.mMatPoints), mnDegree(oldOperation.mnDegree), mbShowPolygon(oldOperation.mbShowPolygon),
      mbShowControls(oldOperation.mbShowControls), mnResolution(oldOperation.mnResolution) {
  mpVOPExternalVariables = oldOperation.mpVOCBSPLineControlPtExtVar ? oldOperation.mpVOCBSPLineControlPtExtVar->Clone()
...
TDVOCBSPLineControlPoint &TDVOCBSPLineControlPoint::operator=(const TDVOCBSPLineControlPoint &oldOperation) {
  ...
  mpObjBSPLine = oldOperation.mpObjBSPLine ? std::make_unique<TDVecBSPLine>(*oldOperation.mpObjBSPLine) : nullptr;
  mpTmpObjBSPLine =
      oldOperation.mpTmpObjBSPLine ? std::make_unique<TDVecBSPLine>(*oldOperation.mpTmpObjBSPLine) : nullptr;
...
TDVOCBSPLineControlPoint *TDVOCBSPLineControlPoint::Clone() const { return new TDVOCBSPLineControlPoint(*this); }
...
void __fastcall TDVOCBSPLineControlPoint::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                      double Y) {
  switch (GetState()) {
  case OSTATE_STARTED:
    if (mMatPoints.empty()) {
...
void __fastcall TDVOCBSPLineControlPoint::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                    double Y) {
  (void)X;
  (void)Y;
...
void __fastcall TDVOCBSPLineControlPoint::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                      double Y) {
  (void)Button;
  (void)Shift;
...
TDVOCBSPLineControlPtExtVar::TDVOCBSPLineControlPtExtVar(TDViewOperation *pParentOperation)
    : TDVOPExternalVariables(pParentOperation), mnDegree(2), mnResolution(50), mbShowControls(true),
      mbShowPolygon(true) {}
...
unsigned int TDVOCBSPLineControlPtExtVar::GetDegree() const { return mnDegree; }

void TDVOCBSPLineControlPtExtVar::SetResolution(unsigned int nResolution) {
  mnResolution = nResolution != 0 ? nResolution : 50;
}

unsigned int TDVOCBSPLineControlPtExtVar::GetResolution() const { return mnResolution; }

void TDVOCBSPLineControlPtExtVar::SetShowControls(bool bShowControls) { mbShowControls = bShowControls; }

bool TDVOCBSPLineControlPtExtVar::GetShowControls() const { return mbShowControls; }

void TDVOCBSPLineControlPtExtVar::SetShowPolygon(bool bShowPolygon) { mbShowPolygon = bShowPolygon; }

bool TDVOCBSPLineControlPtExtVar::GetShowPolygon() const { return mbShowPolygon; }
