...
TDVOMMoveNode::TDVOMMoveNode(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                             TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), mpObject(nullptr), mbDragOK(false), miNode(-1) {
...
TDVOMMoveNode::TDVOMMoveNode(const TDVOMMoveNode &oldOperation)
    : TDVOModify(oldOperation), mpObject(oldOperation.mpObject ? oldOperation.mpObject->Clone() : nullptr),
      mbDragOK(oldOperation.mbDragOK), miNode(oldOperation.miNode) {}
...
TDVOMMoveNode *TDVOMMoveNode::Clone() const { return new TDVOMMoveNode(*this); }
...
void __fastcall TDVOMMoveNode::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X, double Y) {
  ...
  const bool clickOnObject =
      graphicEngine ? selectedObject->HitTest(graphicEngine, point, graphicEngine->GetMouseTolerance()).IsHit()
                    : selectedObject->HitTest(point, MouseToleranceReal()).IsHit();
  miNode = graphicEngine ? selectedObject->PointOnNode(graphicEngine, X, Y, graphicEngine->GetMouseTolerance())
