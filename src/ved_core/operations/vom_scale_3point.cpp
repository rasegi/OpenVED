...
TDVOMScaleObject3Point::TDVOMScaleObject3Point(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                               TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), mpCloneObject(nullptr), mpTmpObject(nullptr),
      mnXScale(1.0), mnYScale(1.0), mnXBasis(0.0), mnYBasis(0.0), MatPoint1{}, MatPoint2{}, mnClick(0),
      mbScaleOK(false) {
...
TDVOMScaleObject3Point::TDVOMScaleObject3Point(const TDVOMScaleObject3Point &oldOperation)
    : TDVOModify(oldOperation),
      ...
      mnXScale(oldOperation.mnXScale), mnYScale(oldOperation.mnYScale), mnXBasis(oldOperation.mnXBasis),
      mnYBasis(oldOperation.mnYBasis), MatPoint1(oldOperation.MatPoint1), MatPoint2(oldOperation.MatPoint2),
      mnClick(oldOperation.mnClick), mbScaleOK(oldOperation.mbScaleOK) {}
...
TDVOMScaleObject3Point *TDVOMScaleObject3Point::Clone() const { return new TDVOMScaleObject3Point(*this); }

void __fastcall TDVOMScaleObject3Point::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                    double Y) {
  (void)Shift;
  switch (GetState()) {
  case OSTATE_STARTED:
...
void __fastcall TDVOMScaleObject3Point::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                  double Y) {
  (void)Button;
  (void)Shift;
  (void)Y;
...
void __fastcall TDVOMScaleObject3Point::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                    double Y) {
  (void)Button;
  (void)Shift;
