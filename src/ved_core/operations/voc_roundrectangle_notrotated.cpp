...
TDMatRect arrangeRect(TDMatRect rect) {
  return {{std::min(rect.P1.x, rect.P3.x), std::min(rect.P1.y, rect.P3.y)},
...
TDMatConturPoint conturPoint(double x, double y, TDConturPointType type) { return {x, y, type, true}; }

std::unique_ptr<TDVecPolyCurveArea> createRoundRectangleArea(const TDMatRect &rect, double radius,
                                                             unsigned int resolution, bool showControls,
                                                             bool showPolygon, TDRgbColor color, bool setColor) {
...
TDVOCRoundRectangleNotRotated::TDVOCRoundRectangleNotRotated(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                                             TDViewOperationManager *pParentOperationManager)
    : TDVOCreate(pVecModel, pVecEditCad, pParentOperationManager), mMatRect{}, mpVOCRoundRectNotRotExtVar(nullptr),
      mpTmpObject(nullptr), mbRectangleOK(false), mnRadius(0.0), mnResolution(5), mbShowControls(false),
      mbShowPolygon(false), mbRectangularLock(true) {
...
TDVOCRoundRectangleNotRotated::TDVOCRoundRectangleNotRotated(const TDVOCRoundRectangleNotRotated &oldOperation)
    : TDVOCreate(oldOperation.mpVecModel, oldOperation.mpVecEditCad, oldOperation.mpParentOperationManager),
      mMatRect(oldOperation.mMatRect), mpVOCRoundRectNotRotExtVar(nullptr),
      mpTmpObject(oldOperation.mpTmpObject ? oldOperation.mpTmpObject->Clone() : nullptr),
      mbRectangleOK(oldOperation.mbRectangleOK), mnRadius(oldOperation.mnRadius),
      mnResolution(oldOperation.mnResolution), mbShowControls(oldOperation.mbShowControls),
      mbShowPolygon(oldOperation.mbShowPolygon), mbRectangularLock(oldOperation.mbRectangularLock) {
...
TDVOCRoundRectangleNotRotated::operator=(const TDVOCRoundRectangleNotRotated &oldOperation) {
...
void __fastcall TDVOCRoundRectangleNotRotated::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                           double Y) {
  (void)Shift;
...
void __fastcall TDVOCRoundRectangleNotRotated::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                         double Y) {
  (void)Shift;
  (void)X;
  (void)Y;
  ...
  if (Button == VIRTMOUSEBTM_LEFT && mbRectangleOK && mClick == 2 && (mPtBeg.x != mPtEnd.x || mPtBeg.y != mPtEnd.y)) {
    ...
    std::unique_ptr<TDVecPolyCurveArea> polyCurveArea = createRoundRectangleArea(
        mMatRect, mnRadius, mnResolution, mbShowControls, mbShowPolygon, mpVecModel->GetDefaultColor(), true);
...
void __fastcall TDVOCRoundRectangleNotRotated::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                           double Y) {
  (void)Shift;
  ...
  if (mClick >= 1) {
    mMatRect = arrangeRect(createOrthogonalRect(mPtBeg, mPtEnd));
    mpTmpObject = createRoundRectangleArea(mMatRect, mnRadius, mnResolution, mbShowControls, mbShowPolygon, 0, false);
...
TDVOCRoundRectNotRotExtVar::TDVOCRoundRectNotRotExtVar(TDViewOperation *pParentOperation)
    : TDVOPExternalVariables(pParentOperation), mnRadius(0.0), mnResolution(5), mbShowControls(false),
      mbShowPolygon(false), mbRectangularLock(true), mbInitialized(false), mnRectWidth(0.0), mnRectHeight(0.0) {}

TDVOCRoundRectNotRotExtVar::TDVOCRoundRectNotRotExtVar(const TDVOCRoundRectNotRotExtVar &oldExtVar)
    : TDVOPExternalVariables(oldExtVar), mnRadius(oldExtVar.mnRadius), mnResolution(oldExtVar.mnResolution),
      mbShowControls(oldExtVar.mbShowControls), mbShowPolygon(oldExtVar.mbShowPolygon),
      mbRectangularLock(oldExtVar.mbRectangularLock), mbInitialized(oldExtVar.mbInitialized),
      mnRectWidth(oldExtVar.mnRectWidth), mnRectHeight(oldExtVar.mnRectHeight) {}
...
TDVOCRoundRectNotRotExtVar *TDVOCRoundRectNotRotExtVar::Clone() const { return new TDVOCRoundRectNotRotExtVar(*this); }

void TDVOCRoundRectNotRotExtVar::SetResolution(unsigned int nResolution) {
  mnResolution = nResolution != 0 ? nResolution : 5;
}

unsigned int TDVOCRoundRectNotRotExtVar::GetResolution() const { return mnResolution; }

void TDVOCRoundRectNotRotExtVar::SetShowControls(bool bShowControls) { mbShowControls = bShowControls; }

bool TDVOCRoundRectNotRotExtVar::GetShowControls() const { return mbShowControls; }

void TDVOCRoundRectNotRotExtVar::SetShowPolygon(bool bShowPolygon) { mbShowPolygon = bShowPolygon; }

bool TDVOCRoundRectNotRotExtVar::GetShowPolygon() const { return mbShowPolygon; }

void TDVOCRoundRectNotRotExtVar::SetRadius(double nRadius) { mnRadius = nRadius; }

double TDVOCRoundRectNotRotExtVar::GetRadius() const { return mnRadius; }

void TDVOCRoundRectNotRotExtVar::SetInitializedFlag(bool bInitialized) { mbInitialized = bInitialized; }

bool TDVOCRoundRectNotRotExtVar::GetInitializedFlag() const { return mbInitialized; }

void TDVOCRoundRectNotRotExtVar::SetRectangularLock(bool bValue) { mbRectangularLock = bValue; }

bool TDVOCRoundRectNotRotExtVar::GetRectangularLock() const { return mbRectangularLock; }

void TDVOCRoundRectNotRotExtVar::SetRectWidth(double nRectWidth) { mnRectWidth = nRectWidth; }

double TDVOCRoundRectNotRotExtVar::GetRectWidth() const { return mnRectWidth; }

void TDVOCRoundRectNotRotExtVar::SetRectHeight(double nRectHeight) { mnRectHeight = nRectHeight; }

double TDVOCRoundRectNotRotExtVar::GetRectHeight() const { return mnRectHeight; }
