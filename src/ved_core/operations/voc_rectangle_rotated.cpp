...
TDVOCRectangleRotated::TDVOCRectangleRotated(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                             TDViewOperationManager *pParentOperationManager)
    : TDVOCreate(pVecModel, pVecEditCad, pParentOperationManager), mMatRect{}, mMatLine{}, mpTmpObject(nullptr),
      mnAngle(0.0), mbRotational(false), mbRectangleOK(false) {
...
TDVOCRectangleRotated::TDVOCRectangleRotated(const TDVOCRectangleRotated &oldOperation)
    : TDVOCreate(oldOperation.mpVecModel, oldOperation.mpVecEditCad, oldOperation.mpParentOperationManager),
      mMatRect(oldOperation.mMatRect), mMatLine(oldOperation.mMatLine),
      mpTmpObject(oldOperation.mpTmpObject ? oldOperation.mpTmpObject->Clone() : nullptr),
      mnAngle(oldOperation.mnAngle), mbRotational(oldOperation.mbRotational),
...
TDVOCRectangleRotated *TDVOCRectangleRotated::Clone() const { return new TDVOCRectangleRotated(*this); }

void __fastcall TDVOCRectangleRotated::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                   double Y) {
  (void)Shift;
...
void __fastcall TDVOCRectangleRotated::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                 double Y) {
  (void)Shift;
  (void)X;
  (void)Y;
...
void __fastcall TDVOCRectangleRotated::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                   double Y) {
  (void)Button;
  (void)Shift;
  ...
  case 2: {
    ...
    if (mbRotational) {
      ...
      if (!MatBelike2Double(mMatRect.P2.x, mMatRect.P3.x, 4) && !MatBelike2Double(mMatRect.P2.y, mMatRect.P3.y, 4)) {
        ...
        const TDMatLine line3{mMatRect.P3.x, mMatRect.P3.y, mMatRect.P3.x + (line1.x2 - line1.x1),
                              mMatRect.P3.y + (line1.y2 - line1.y1)};
        const TDMatLine line4{mMatRect.P1.x, mMatRect.P1.y, mMatRect.P1.x + (line2.x2 - line2.x1),
