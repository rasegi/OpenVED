...
double distanceToSegment(TDMatPoint point, TDMatPoint start, TDMatPoint end) {
  ...
  const double t =
      std::max(0.0, std::min(1.0, ((point.x - start.x) * dx + (point.y - start.y) * dy) / (dx * dx + dy * dy)));
...
TDVOMInsertNode::TDVOMInsertNode(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                 TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), meObjectType(VECOBJ_UNKNOWN), mpCloneObject(nullptr),
      mpTmpObject(nullptr), mbInsertOK(false), mbObjectTypeOK(false), bOnObject(false), miNode(-1), miVorNode(-1) {
...
TDVOMInsertNode::TDVOMInsertNode(const TDVOMInsertNode &oldOperation)
    : TDVOModify(oldOperation), meObjectType(oldOperation.meObjectType),
      ...
      mbInsertOK(oldOperation.mbInsertOK), mbObjectTypeOK(oldOperation.mbObjectTypeOK),
      bOnObject(oldOperation.bOnObject), miNode(oldOperation.miNode), miVorNode(oldOperation.miVorNode) {}
...
TDVOMInsertNode *TDVOMInsertNode::Clone() const { return new TDVOMInsertNode(*this); }
...
long TDVOMInsertNode::PointOnNode(TDVecObject *object, TDMatPoint point) const {
  ...
  if (auto *bspline = dynamic_cast<TDVecBSPLine *>(object)) {
    return graphicEngine ? bspline->PointOnControle(graphicEngine, point.x, point.y, graphicEngine->GetMouseTolerance())
                         : object->PointOnNode(point, MouseToleranceReal());
  }
  return graphicEngine ? object->PointOnNode(graphicEngine, point.x, point.y, graphicEngine->GetMouseTolerance())
                       : object->PointOnNode(point, MouseToleranceReal());
}
...
long TDVOMInsertNode::PointAfterNode(TDVecObject *object, TDMatPoint point) const {
  ...
  if (auto *bspline = dynamic_cast<TDVecBSPLine *>(object)) {
    return graphicEngine
               ...
               : pointAfterFromPoints(
                     bspline->CountPoints(), [bspline](int i) { return bspline->GetPoint(i); }, point,
                     MouseToleranceReal());
  }
  if (auto *polygon = dynamic_cast<TDVecPolygon *>(object)) {
    return graphicEngine ? polygon->PointAfterNode(graphicEngine, point.x, point.y, graphicEngine->GetMouseTolerance())
                         : pointAfterFromPoints(
                               polygon->CountPoints(), [polygon](int i) { return polygon->GetPoint(i); }, point,
                               MouseToleranceReal());
  }
  if (auto *polygonArea = dynamic_cast<TDVecPolygonArea *>(object)) {
    return graphicEngine
               ...
               : pointAfterFromPoints(
                     polygonArea->CountPoints(), [polygonArea](int i) { return polygonArea->GetPoint(i); }, point,
                     MouseToleranceReal());
  }
...
bool TDVOMInsertNode::InsertPointInto(TDVecObject *object, long afterNode, TDMatPoint point, TDOPVirtKeyState shift) {
  ...
  const TDMatConturPoint conturPoint{point.x, point.y, (shift == VKState_KEY_CTRL ? CPT_PRIME_QSPLINE : CPT_PRIME_LINE),
                                     true};
  if (auto *polyCurve = dynamic_cast<TDVecPolyCurve *>(object)) {
...
void __fastcall TDVOMInsertNode::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X, double Y) {
  ...
  TDGraphicEngine *graphicEngine = mpVecEditCad->GetActiveGraphicEngine();
  bOnObject = graphicEngine ? selectedObject->HitTest(graphicEngine, point, graphicEngine->GetMouseTolerance()).IsHit()
