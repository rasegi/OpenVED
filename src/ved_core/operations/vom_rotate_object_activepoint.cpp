...
TDVOMRotateObjectActivePoint::TDVOMRotateObjectActivePoint(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                                           TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), mpVOMRotateObjectActPtExtVar(nullptr), mTmpObjects(),
      mShadowObjects(), mbRotateOK(false), mbRotated(false), mnAngle(0.0), mbSelect(false), mbCopy(false) {
...
TDVOMRotateObjectActivePoint::TDVOMRotateObjectActivePoint(const TDVOMRotateObjectActivePoint &oldOperation)
    : TDVOModify(oldOperation), mpVOMRotateObjectActPtExtVar(nullptr), mTmpObjects(), mShadowObjects(),
      mbRotateOK(oldOperation.mbRotateOK), mbRotated(oldOperation.mbRotated), mnAngle(oldOperation.mnAngle),
      mbSelect(oldOperation.mbSelect), mbCopy(oldOperation.mbCopy) {
...
TDVOMRotateObjectActivePoint::operator=(const TDVOMRotateObjectActivePoint &oldOperation) {
...
void __fastcall TDVOMRotateObjectActivePoint::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                          double Y) {
  (void)Shift;
  (void)X;
  (void)Y;
...
void __fastcall TDVOMRotateObjectActivePoint::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                        double Y) {
  (void)Shift;
...
void __fastcall TDVOMRotateObjectActivePoint::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                          double Y) {
  (void)Button;
  (void)Shift;
...
TDVOMRotateObjectActPtExtVar::TDVOMRotateObjectActPtExtVar(TDViewOperation *pParentOperation)
    : TDVOPExternalVariables(pParentOperation), mnObjects(0), mnAngle(0.0), mPivotPoint{0.0, 0.0}, mbCopy(false),
      mbSelect(false), mbInitialized(false) {}

TDVOMRotateObjectActPtExtVar::TDVOMRotateObjectActPtExtVar(const TDVOMRotateObjectActPtExtVar &oldExtVar)
    : TDVOPExternalVariables(oldExtVar), mnObjects(oldExtVar.mnObjects), mnAngle(oldExtVar.mnAngle),
      mPivotPoint(oldExtVar.mPivotPoint), mbCopy(oldExtVar.mbCopy), mbSelect(oldExtVar.mbSelect),
...
void TDVOMRotateObjectActPtExtVar::SetAngle(double nAngle) { mnAngle = nAngle; }

double TDVOMRotateObjectActPtExtVar::GetAngle() const { return mnAngle; }

void TDVOMRotateObjectActPtExtVar::SetPivotPoint(TDMatPoint pivotPoint) { mPivotPoint = pivotPoint; }

TDMatPoint TDVOMRotateObjectActPtExtVar::GetPivotPoint() const { return mPivotPoint; }

void TDVOMRotateObjectActPtExtVar::SetCopyFlag(bool bCopy) { mbCopy = bCopy; }

bool TDVOMRotateObjectActPtExtVar::GetCopyFlag() const { return mbCopy; }

void TDVOMRotateObjectActPtExtVar::SetNumberOfObjects(unsigned long nObjects) { mnObjects = nObjects; }

unsigned long TDVOMRotateObjectActPtExtVar::GetNumberOfObjects() const { return mnObjects; }

void TDVOMRotateObjectActPtExtVar::SetSelectFlag(bool bSelect) { mbSelect = bSelect; }

bool TDVOMRotateObjectActPtExtVar::GetSelectFlag() const { return mbSelect; }

void TDVOMRotateObjectActPtExtVar::SetInitializedFlag(bool bInitialized) { mbInitialized = bInitialized; }

bool TDVOMRotateObjectActPtExtVar::GetInitializedFlag() const { return mbInitialized; }
