...
TDVOMSelectMoveNodeObject::TDVOMSelectMoveNodeObject(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                                     TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), miObjectIndex(-1), mpObject(nullptr), mnNodeMove(-1),
      mMouseStart{0.0, 0.0}, mMouseCurrent{0.0, 0.0}, mbMoveObject(false), mbMoveNode(false), mbGroupMove(false),
      mbObjectChanged(false), mbToleranceOK(false), mbAreaSelect(false), mbAreaMoved(false) {
...
TDVOMSelectMoveNodeObject::TDVOMSelectMoveNodeObject(const TDVOMSelectMoveNodeObject &oldOperation)
    : TDVOModify(oldOperation), miObjectIndex(oldOperation.miObjectIndex), mpObject(oldOperation.mpObject),
      mnNodeMove(oldOperation.mnNodeMove), mMouseStart(oldOperation.mMouseStart),
      mMouseCurrent(oldOperation.mMouseCurrent), mbMoveObject(oldOperation.mbMoveObject),
      mbMoveNode(oldOperation.mbMoveNode), mbGroupMove(oldOperation.mbGroupMove),
      mbObjectChanged(oldOperation.mbObjectChanged), mbToleranceOK(oldOperation.mbToleranceOK),
      mbAreaSelect(oldOperation.mbAreaSelect), mbAreaMoved(oldOperation.mbAreaMoved) {
...
TDVOMSelectMoveNodeObject *TDVOMSelectMoveNodeObject::Clone() const { return new TDVOMSelectMoveNodeObject(*this); }
...
void __fastcall TDVOMSelectMoveNodeObject::OPMouseDown(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                       double Y) {
  switch (GetState()) {
  case OSTATE_UNKNOWN:
  case OSTATE_ABORTED:
...
void __fastcall TDVOMSelectMoveNodeObject::OPMouseUp(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                     double Y) {
  if (Button != VIRTMOUSEBTM_LEFT) {
    ResetInteractiveState();
    mpVecEditCad->UseCursor(VECVIEW_CURSOR_DEFAULT);
...
void __fastcall TDVOMSelectMoveNodeObject::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X,
                                                       double Y) {
  (void)Shift;
  if (Button != VIRTMOUSEBTM_LEFT) {
    return;
  ...
  if (mbAreaSelect) {
    ...
    if (std::fabs(point.x - mMouseStart.x) > tolerance || std::fabs(point.y - mMouseStart.y) > tolerance) {
      mbAreaMoved = true;
    }
  }
}
