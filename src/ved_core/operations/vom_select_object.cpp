...
TDVOMSelectObject::TDVOMSelectObject(TDVecModel *pVecModel, TDVecEditCad *pVecEditCad,
                                     TDViewOperationManager *pParentOperationManager)
    : TDVOModify(pVecModel, pVecEditCad, pParentOperationManager), miSelectObject(-1), mpSelectObject(nullptr),
      mAreaStart{0.0, 0.0}, mAreaCurrent{0.0, 0.0}, mbAreaSelect(false), mbAreaMoved(false) {
...
TDVOMSelectObject::TDVOMSelectObject(const TDVOMSelectObject &oldOperation)
    : TDVOModify(oldOperation), miSelectObject(oldOperation.miSelectObject),
      mpSelectObject(oldOperation.mpSelectObject), mAreaStart(oldOperation.mAreaStart),
      mAreaCurrent(oldOperation.mAreaCurrent), mbAreaSelect(oldOperation.mbAreaSelect),
...
TDVOMSelectObject *TDVOMSelectObject::Clone() const { return new TDVOMSelectObject(*this); }
...
void __fastcall TDVOMSelectObject::OPMouseMove(TDOPVirtMouseButton Button, TDOPVirtKeyState Shift, double X, double Y) {
  ...
  if (std::fabs(mAreaCurrent.x - mAreaStart.x) > tolerance || std::fabs(mAreaCurrent.y - mAreaStart.y) > tolerance) {
    mbAreaMoved = true;
  }
}
