...
TDGraphicEngineQt::TDGraphicEngineQt() : painter_(nullptr), drawColor_(0), nodeSize_(3) { LoadDefaultNodeImages(); }

void TDGraphicEngineQt::SetPainter(QPainter *painter) { painter_ = painter; }

QPainter *TDGraphicEngineQt::Painter() const { return painter_; }
...
void TDGraphicEngineQt::SetNodeImages(std::vector<QPixmap> nodeImages) { nodeImages_ = std::move(nodeImages); }

TDGraphicEngineScreenState &TDGraphicEngineQt::ScreenState() { return screenState_; }

const TDGraphicEngineScreenState &TDGraphicEngineQt::ScreenState() const { return screenState_; }
...
void TDGraphicEngineQt::DrawLine(const TDMatLine *pParams, bool bOutLine) {
  ...
  painter_->drawLine(QPointF(RealToXPos(pParams->x1), RealToYPos(pParams->y1)),
...
void TDGraphicEngineQt::DrawLineOutLine(const TDMatLine *pParams) {
  ...
  painter_->drawLine(QPointF(MatRound(pParams->x1), MatRound(pParams->y1)),
...
void TDGraphicEngineQt::DrawBoxOutLine(TDMatPoint MatPoint1, TDMatPoint MatPoint2) {
  ...
  const QRectF rect(QPointF(RealToXPos(MatRound(MatPoint1.x)), RealToYPos(MatRound(MatPoint1.y))),
...
void TDGraphicEngineQt::DrawRulers(const TDVecMeasureScale &scale, const TDVecUnitFormatter &formatter) {
  ...
  const QRect clip = painter_->hasClipping() ? painter_->clipBoundingRect().toAlignedRect() : painter_->viewport();
  ...
  for (long x = firstX; x <= lastX; x += rulerStep) {
    ...
    if (isMajor && xScreen - lastHorizontalLabelRight >= kMinimumLabelDistance) {
      ...
      painter_->drawText(xScreen + kLabelPadding, horizontalRuler.top() + painter_->fontMetrics().ascent() + 2, label);
...
void TDGraphicEngineQt::DrawNode(double x, double y, TDNodeType eNodeType, bool bLock) {
  ...
  QRectF rect(RealToXPos(x) - halfSize, RealToYPos(y) - halfSize, std::max(1, nodeSize_), std::max(1, nodeSize_));
...
long TDGraphicEngineQt::RealToXPos(double x) const { return screenState_.RealToXPos(x); }

long TDGraphicEngineQt::RealToYPos(double y) const { return screenState_.RealToYPos(y); }

long TDGraphicEngineQt::RealToXPos(long x) const { return screenState_.RealToXPos(x); }

long TDGraphicEngineQt::RealToYPos(long y) const { return screenState_.RealToYPos(y); }

long TDGraphicEngineQt::RealToXVal(double x) const { return screenState_.RealToXVal(x); }

long TDGraphicEngineQt::RealToYVal(double y) const { return screenState_.RealToYVal(y); }

long TDGraphicEngineQt::RealToXVal(long x) const { return screenState_.RealToXVal(x); }

long TDGraphicEngineQt::RealToYVal(long y) const { return screenState_.RealToYVal(y); }

double TDGraphicEngineQt::ScreenToXPos(long x) const { return screenState_.ScreenToXPos(x); }

double TDGraphicEngineQt::ScreenToYPos(long y) const { return screenState_.ScreenToYPos(y); }

double TDGraphicEngineQt::ScreenToXVal(long x) const { return screenState_.ScreenToXVal(x); }

double TDGraphicEngineQt::ScreenToYVal(long y) const { return screenState_.ScreenToYVal(y); }

void TDGraphicEngineQt::SetDrawColor(TDRgbColor nColor) { drawColor_ = nColor; }

TDRgbColor TDGraphicEngineQt::GetDrawColor() const { return drawColor_; }

int TDGraphicEngineQt::GetNodeSize() const { return nodeSize_; }

void TDGraphicEngineQt::SetNodeSize(int nSize) { nodeSize_ = std::max(1, nSize); }

bool TDGraphicEngineQt::IsRectVisible(TDMatRect MatRect) const { return screenState_.IsRectVisible(MatRect); }

long TDGraphicEngineQt::GetMouseTolerance() const { return screenState_.GetMouseTolerance(); }

double TDGraphicEngineQt::GetMinimumDistance() const { return screenState_.GetMinimumDistance(); }

void TDGraphicEngineQt::SetPanX(long nPanX) { screenState_.SetPanX(nPanX); }

void TDGraphicEngineQt::SetPanY(long nPanY) { screenState_.SetPanY(nPanY); }
...
void TDGraphicEngineQt::ChangePanX(long xDiff) { screenState_.ChangePanX(xDiff); }

void TDGraphicEngineQt::ChangePanY(long yDiff) { screenState_.ChangePanY(yDiff); }

long TDGraphicEngineQt::GetPanX() const { return screenState_.GetPanX(); }

long TDGraphicEngineQt::GetPanY() const { return screenState_.GetPanY(); }

double TDGraphicEngineQt::GetMarginX() const { return screenState_.GetMarginX(); }

double TDGraphicEngineQt::GetMarginY() const { return screenState_.GetMarginY(); }

void TDGraphicEngineQt::SetMarginX(double x) { screenState_.SetMarginX(x); }

void TDGraphicEngineQt::SetMarginY(double y) { screenState_.SetMarginY(y); }

void TDGraphicEngineQt::SetMinimumDistance(long nPixels) { screenState_.SetMinimumDistance(nPixels); }

void TDGraphicEngineQt::SetInchPerReal(double nNewInchPerReal) { screenState_.SetInchPerReal(nNewInchPerReal); }

double TDGraphicEngineQt::GetInchPerReal() const { return screenState_.GetInchPerReal(); }
...
void TDGraphicEngineQt::SetViewRange(const TDCoordinateSystem2D &newRange) { screenState_.SetViewRange(newRange); }
...
const TDCoordinateSystem2D &TDGraphicEngineQt::GetViewRange() const { return screenState_.GetViewRange(); }

const TDCoordinateSystem2D &TDGraphicEngineQt::GetWorldSpaceRange() const { return screenState_.GetWorldSpaceRange(); }

const TDCoordinateSystem2D &TDGraphicEngineQt::GetWorkSpaceRange() const { return screenState_.GetWorkSpaceRange(); }

double TDGraphicEngineQt::GetZoom() const { return screenState_.GetZoom(); }

void TDGraphicEngineQt::SetZoom(double nNewZoom) { screenState_.SetZoom(nNewZoom); }

void TDGraphicEngineQt::SetZoomAtScreenPoint(double nNewZoom, long x, long y) {
  screenState_.SetZoomAtScreenPoint(nNewZoom, x, y);
}

void TDGraphicEngineQt::SetZoom(TDFraction newZoom) { screenState_.SetZoom(newZoom); }

void TDGraphicEngineQt::SetZoomToRealSize() { screenState_.SetZoomToRealSize(); }

void TDGraphicEngineQt::SetViewSize(TDRecSize recSize) { screenState_.SetViewSize(recSize); }

void TDGraphicEngineQt::SetViewSize(long width, long height) { screenState_.SetViewSize(width, height); }
...
void TDGraphicEngineQt::DrawAuxLineX(long yPos) {
  ...
  const QRect clip =
      painter_->clipBoundingRect().isEmpty() ? painter_->viewport() : painter_->clipBoundingRect().toAlignedRect();
...
void TDGraphicEngineQt::DrawGrid(long nWidth, long nHeight, long nDist, long nSubDiv, TDRgbColor nColor,
                                 long nResLimit) {
  if (!painter_) {
    return;
  }
...
void TDGraphicEngineQt::DrawMouseToleranz(double x, double y) {
  ...
  painter_->setPen(drawPen(false));
  painter_->setBrush(Qt::NoBrush);
  painter_->drawRoundedRect(QRectF(xScreen - tolerance, yScreen - tolerance, tolerance * 2, tolerance * 2), tolerance,
                            tolerance);
  painter_->restore();
}
...
QPen TDGraphicEngineQt::drawPen(bool outline) const { return makePen(drawColor_, outline); }

QPen TDGraphicEngineQt::auxLinePen() const { return makePen(drawColor_, true); }
