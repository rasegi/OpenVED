...
struct FreeTypeFace {
  ...
  FreeTypeFace(FT_Library library, const char *path, long faceIndex) { FT_New_Face(library, path, faceIndex, &face); }
...
TDMatConturPoint toVedPoint(const FT_Vector *point, double scale, double ascent) {
  return {static_cast<double>(point->x) * scale, (ascent - static_cast<double>(point->y)) * scale, CPT_PRIME_LINE,
          true};
}
...
FT_Vector cubicPoint(const FT_Vector &p0, const FT_Vector &p1, const FT_Vector &p2, const FT_Vector &p3, double t) {
  ...
  return {static_cast<FT_Pos>(std::lround(a * p0.x + b * p1.x + c * p2.x + d * p3.x)),
...
int cubicToCallback(const FT_Vector *control1, const FT_Vector *control2, const FT_Vector *to, void *user) {
  ...
  const FT_Vector from{static_cast<FT_Pos>(std::lround(context->last.x / context->scale)),
...
std::unique_ptr<TDVecGlyph> convertGlyphOutline(FT_Face face, FT_UInt glyphIndex, double scale, double ascent) {
  ...
  if (face->glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
    const FT_Outline_Funcs callbacks{moveToCallback, lineToCallback, conicToCallback, cubicToCallback, 0, 0};
...
double FaceScale(FT_Face face) { return faceScale(face); }

std::unique_ptr<TDVecGlyph> ConvertGlyphOutline(FT_Face face, FT_UInt glyphIndex, double scale, double ascent) {
  return convertGlyphOutline(face, glyphIndex, scale, ascent);
}

static std::unique_ptr<TDVecFont> convertFaceToVecFont(FT_Face face, const std::string &fontName,
  ...
  if (coverage == CharacterCoverage::FullCmap) {
    ...
    for (FT_ULong codePoint = FT_Get_First_Char(face, &glyphIndex); glyphIndex != 0;
    ...
    for (int character = kFirstConvertedCharacter; character <= kLastConvertedCharacter; ++character) {
      appendCharacter(static_cast<unsigned int>(character), FT_Get_Char_Index(face, static_cast<FT_ULong>(character)));
    }
  }
...
std::unique_ptr<TDVecFont> ConvertTrueTypeFileToVecFont(const std::string &encodedPath, long faceIndex,
                                                        const std::string &fontName, CharacterCoverage coverage) {
...
std::unique_ptr<TDVecFont> ConvertTrueTypeMemoryToVecFont(const void *data, long size, long faceIndex,
  ...
  FT_Face face = nullptr;
  if (FT_New_Memory_Face(freeType.library, static_cast<const FT_Byte *>(data), static_cast<FT_Long>(size), faceIndex,
                         &face) != 0 ||
      !face) {
    return nullptr;
  }
